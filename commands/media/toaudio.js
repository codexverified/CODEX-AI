...
module.exports = {
    ...
    run: async (sock, message, args, { jid, contextInfo, reply }) => {
        ...
        if (!target || !mediaType) {
            return reply(fmt('âŒ Reply to a *video* or *audio/voice* message with `.toaudio`'));
        }
 
        await sock.sendMessage(jid, { text: fmt('â³ Converting to audioâ€¦'), contextInfo }, { quoted: message });
        ...
        try {
            ...
            await reply(fmt(`âŒ Conversion failed: ${err.message}`));
        } finally {
