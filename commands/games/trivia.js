...
module.exports = {
    ...
    run: async (sock, message, args, { sender, groupId, contextInfo }) => {
        ...
        if (arg === 'answer' || arg === 'a') {
            ...
            if (!session) return sock.sendMessage(chatId, { text: 'â“ No active trivia. Start one with .trivia', contextInfo }, { quoted: message });
            ...
            return sock.sendMessage(chatId, {
                text: isRight
                    ? `âœ… *Correct!* ðŸŽ‰ The answer is: *${session.answer}*`
                    : `âŒ *Wrong!* The correct answer was: *${session.answer}*`,
                contextInfo
            }, { quoted: message });
        }
        try {
            ...
            setTimeout(() => {
                if (sessions.has(chatId)) {
                    sessions.delete(chatId);
                    sock.sendMessage(chatId, { text: `â° *Time's up!* The answer was: *${correct}*` }).catch(() => {});
                }
            }, 30000);
            const optText = options.map((o, i) => `${labels[i]}. ${o}`).join('\n');
            await sock.sendMessage(chatId, {
                text:
`â“ *Trivia â€” ${decode(q.category)}*
ðŸŽ¯ Difficulty: ${q.difficulty}
 
${question}
 
${optText}
 
ðŸ’¡ Reply: .trivia a <A/B/C/D>  (30 seconds!)`,
                contextInfo
            }, { quoted: message });
        } catch (e) {
            await sock.sendMessage(chatId, { text: `âŒ Trivia failed: ${e.message}`, contextInfo }, { quoted: message });
        }
    }
};
