    ...
    run: async (sock, message, args, ctx) => {
        ...
        if (!url) {
            return sock.sendMessage(jid, {
                text: 'ðŸ”— *Link Summarizer*\n\nSummarize any article or webpage!\n\n*Usage:*\nâ€¢ `.summarize https://example.com/article`\nâ€¢ Reply to a message containing a link with `.summarize`\n\n_Extracts title, description, and key content._',
                contextInfo
            }, { quoted: message });
        }
 
        await sock.sendMessage(jid, { text: 'ðŸ” Fetching and summarizing...', contextInfo }, { quoted: message });
        ...
        try {
            ...
            if (!meta.title && !meta.description && !meta.excerpt) {
                return sock.sendMessage(jid, {
                    text: 'âŒ Could not extract content from this URL. The page may be dynamic or protected.',
                    contextInfo
                }, { quoted: message });
            }
 
            let summary = `ðŸ”— *Link Summary*\n\n`;
            if (meta.title) summary += `ðŸ“° *Title:* ${meta.title}\n`;
            if (meta.siteName) summary += `ðŸŒ *Site:* ${meta.siteName}\n`;
            if (meta.author) summary += `âœï¸ *Author:* ${meta.author}\n`;
            if (meta.type) summary += `ðŸ“‚ *Type:* ${meta.type}\n`;
            summary += `\n`;
            if (meta.description) summary += `ðŸ“ *Description:*\n${meta.description}\n\n`;
            if (meta.excerpt) summary += `ðŸ“– *Excerpt:*\n${meta.excerpt}\n`;
            summary += `\nðŸ”— ${url}`;
 
            return sock.sendMessage(jid, { text: summary, contextInfo }, { quoted: message });
        } catch (err) {
            return sock.sendMessage(jid, {
                text: `âŒ Failed to fetch the link. Error: ${err.message}`,
                contextInfo
            }, { quoted: message });
        }
    }
