...
module.exports = {
    ...
    run: async (sock, message, args, ctx) => {
        ...
        if (cmd === 'imgbb') {
            if (!hasMedia) return send('âŒ Send or reply to an *image* with `.imgbb` to upload it to ImgBB.');
            try {
                ...
                if (url) return send(`âœ… *Image Uploaded!*\n\nðŸ”— ${url}\n\n_To use ImgBB: set IMGBB_KEY env var_`);
                return send('âŒ Upload failed. Set IMGBB_KEY env var for ImgBB support.');
            } catch { return send('âŒ Upload failed. Try `.catbox` instead.'); }
        }
 
        if (cmd === 'pixhost') {
            if (!hasMedia) return send('âŒ Send or reply to an *image* with `.pixhost` to upload it to Pixhost.');
            try {
                await sock.sendPresenceUpdate('composing', jid);
                const buf  = await dlBuffer(imgMsg || docMsg, imgMsg ? 'image' : 'document');
                const FormData = require('form-data');
                ...
                if (imgUrl) {
                    return send(`âœ… *Uploaded to Pixhost!*\n\nðŸ”— View: ${imgUrl}\nðŸ–¼ï¸ Direct: ${thumbUrl || imgUrl}`);
                }
                return send('âŒ Pixhost upload failed.');
            } catch { return send('âŒ Pixhost upload failed. Try `.catbox` instead.'); }
        }
 
        if (cmd === 'githubcdn') {
            const token = process.env.GITHUB_TOKEN;
            const repo  = process.env.GITHUB_REPO || 'user/bot-cdn';
            if (!token) return send('âš ï¸ *GitHub CDN*\n\nSet `GITHUB_TOKEN` and `GITHUB_REPO` env vars to use this feature.\n\nAlternative: Use `.catbox` for free image hosting.');
            if (!hasMedia) return send('âŒ Send or reply to an image with `.githubcdn`');
            try {
                await sock.sendPresenceUpdate('composing', jid);
                const buf      = await dlBuffer(imgMsg || docMsg, imgMsg ? 'image' : 'document');
                ...
                return send(`âœ… *GitHub CDN Upload!*\n\nðŸ”— ${rawUrl}`);
            } catch { return send('âŒ GitHub CDN upload failed. Check GITHUB_TOKEN and GITHUB_REPO.'); }
        }
    }
};
