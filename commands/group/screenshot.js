    ...
    run: async (sock, message, args, { sender, contextInfo }) => {
        ...
        if (!url) {
            return sock.sendMessage(sender, {
                text: 'ðŸŒ Please provide a URL.\nExample: .ss https://google.com',
                contextInfo
            }, { quoted: message });
        }
        if (!/^https?:\/\//.test(url)) url = 'https://' + url;
        await sock.sendMessage(sender, { text: 'â³ Taking screenshot...', contextInfo }, { quoted: message });
        try {
            ...
            await sock.sendMessage(sender, { text: `âŒ Screenshot failed: ${e.message}`, contextInfo }, { quoted: message });
        }
    }
};
