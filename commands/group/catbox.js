...
module.exports = {
    ...
    run: async (sock, message, args, { jid, contextInfo, reply }) => {
        ...
        if (!msgKey) {
            return reply(fmt('âŒ Reply to (or send with) an *image, video, audio, or document* and use this command.'));
        }
        ...
        await sock.sendMessage(jid, { text: fmt('â³ Uploading to Catboxâ€¦'), contextInfo }, { quoted: message });
        ...
        try {
            ...
            const label = mimeType.includes('image') ? 'ðŸ–¼ï¸ Image'
                : mimeType.includes('video')  ? 'ðŸŽ¬ Video'
                : mimeType.includes('audio')  ? 'ðŸŽµ Audio'
                : 'ðŸ“„ File';
            ...
            await reply(fmt(`âš ï¸ Upload failed: ${err.message}`));
        } finally {
