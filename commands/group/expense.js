...
module.exports = {
    ...
    run: async (sock, message, args, ctx) => {
        ...
        if (rawCmd === 'clearexpenses') {
            ...
            return sock.sendMessage(jid, { text: 'ðŸ—‘ï¸ *All expenses cleared.*', contextInfo }, { quoted: message });
        }
        ...
        if (rawCmd === 'addexpense') {
            ...
            if (isNaN(amount) || amount <= 0) {
                return sock.sendMessage(jid, {
                    text: 'âŒ *Usage:* `.addexpense <amount> <description>`\n\n*Example:* `.addexpense 500 Lunch at Java`',
                    contextInfo
                }, { quoted: message });
            }
            ...
            return sock.sendMessage(jid, {
                text: `ðŸ’° *Expense Added!*\n\nðŸ’µ *Amount:* ${group.currency} ${amount.toLocaleString()}\nðŸ“ *Description:* ${desc}\nðŸ‘¤ *Paid by:* @${num}\n\nðŸ“Š *Group Total:* ${group.currency} ${total.toLocaleString()}`,
                mentions: [sender],
                contextInfo
            }, { quoted: message });
        }
        ...
        if (rawCmd === 'splitbill') {
            ...
            if (isNaN(amount) || amount <= 0 || isNaN(people) || people < 2) {
                return sock.sendMessage(jid, {
                    text: 'âŒ *Usage:* `.splitbill <total> <people> [description]`\n\n*Example:* `.splitbill 1200 4 Dinner`',
                    contextInfo
                }, { quoted: message });
            }
            ...
            return sock.sendMessage(jid, {
                text: `ðŸ’³ *Bill Split*\n\nðŸ“ *${desc}*\nðŸ’µ *Total:* ${group.currency} ${amount.toLocaleString()}\nðŸ‘¥ *People:* ${people}\n\nðŸ’° *Each pays:* ${group.currency} ${parseFloat(perPerson).toLocaleString()}\n\n_Use \`.addexpense ${perPerson} ${desc}\` to log your share._`,
                contextInfo
            }, { quoted: message });
        }
        ...
        if (rawCmd === 'settle') {
            ...
            group.entries.push({
                amount: -amount,
                description: `Settlement: @${fromNum} â†’ @${toNum}`,
                paidBy: sender,
                paidByNum: fromNum,
                settledWith,
                timestamp: Date.now(),
        ...
        if (rawCmd === 'balances') {
            ...
            if (!sorted.length) {
                return sock.sendMessage(jid, { text: 'ðŸ“Š No expenses recorded yet.\n\nAdd one with `.addexpense 500 Lunch`', contextInfo }, { quoted: message });
            }
            ...
            const balances = sorted.map(([user, spent]) => {
                ...
                return `@${num}: Spent ${group.currency} ${spent.toLocaleString()} â†’ ${status}`;
            }).join('\n');
 
            const mentions = sorted.map(([user]) => user);
 
            return sock.sendMessage(jid, {
                text: `ðŸ“Š *Group Balances*\n\nðŸ’µ *Total spent:* ${group.currency} ${totalSpent.toLocaleString()}\nðŸ‘¤ *Average:* ${group.currency} ${avg.toFixed(0)}\n\n${balances}\n\n_Settle up: \`.settle 250 @person\`_`,
                mentions,
                contextInfo
            }, { quoted: message });
        }
        ...
        if (['expenses', 'expense'].includes(rawCmd)) {
            if (!group.entries.length) {
                return sock.sendMessage(jid, {
                    text: 'ðŸ’° *Expense Tracker*\n\nNo expenses yet!\n\n*Commands:*\nâ€¢ `.addexpense 500 Lunch`\nâ€¢ `.splitbill 1200 4 Dinner`\nâ€¢ `.balances`\nâ€¢ `.settle 250 @user`\nâ€¢ `.clearexpenses`',
                    contextInfo
                }, { quoted: message });
            }
            ...
            const list = recent.map((e, i) => {
                ...
                const sign = e.type === 'settlement' ? 'ðŸ”„' : 'ðŸ’µ';
                return `${i + 1}. ${sign} ${group.currency} ${Math.abs(e.amount).toLocaleString()} â€” ${e.description}\n   _@${e.paidByNum} â€¢ ${date}_`;
            }).join('\n\n');
 
            const total = group.entries.reduce((s, e) => s + e.amount, 0);
 
            return sock.sendMessage(jid, {
                text: `ðŸ’° *Recent Expenses*\n\n${list}\n\nðŸ“Š *Total:* ${group.currency} ${total.toLocaleString()}\n\n_See all balances: \`.balances\`_`,
                contextInfo
            }, { quoted: message });
        }
    }
