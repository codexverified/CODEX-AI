    ...
    run: async (sock, message, args, { sender, contextInfo }) => {
        ...
        if (!hex || !/^[0-9a-fA-F]{3,6}$/.test(hex)) {
            return sock.sendMessage(sender, {
                text: 'ðŸŽ¨ Please provide a valid HEX color.\nExample: .color ff5733',
                contextInfo
            }, { quoted: message });
        }
...
â–¸ *Name:*  ${data.name.value}
â–¸ *HEX:*   #${hex.toUpperCase()}
â–¸ *RGB:*   rgb(${r}, ${g}, ${b})
â–¸ *HSL:*   hsl(${hsl.h}Â°, ${hsl.s}%, ${hsl.l}%)
â–¸ *Closest:* ${data.name.closest_named_hex}
 
_Powered by CODEX AI_`,
                contextInfo
            }, { quoted: message });
        } catch (e) {
            await sock.sendMessage(sender, { text: `âŒ Color lookup failed: ${e.message}`, contextInfo }, { quoted: message });
        }
    }
};
