...
module.exports = {
    ...
    run: async (sock, message, args, ctx) => {
        ...
        if (!isAdmin) {
            return sock.sendMessage(jid, { text: fmt(theme.admin || 'â›” Only admins can lock or unlock the group.'), contextInfo }, { quoted: message });
        }
        if (!isBotAdmin) {
            return sock.sendMessage(jid, { text: fmt(theme.botAdmin || 'â›” I need to be an admin to change group settings.'), contextInfo }, { quoted: message });
        }
 
        if (isLock) {
            await sock.groupSettingUpdate(jid, 'announcement');
            await sock.sendMessage(jid, {
                text: fmt('ðŸ”’ *Group locked.*\nOnly admins can send messages now.'),
                contextInfo
            }, { quoted: message });
        } else {
            await sock.groupSettingUpdate(jid, 'not_announcement');
            await sock.sendMessage(jid, {
                text: fmt('ðŸ”“ *Group unlocked.*\nEveryone can send messages now.'),
                contextInfo
            }, { quoted: message });
        }
    }
