...
module.exports = {
    ...
    run: async (sock, message, args, ctx) => {
        ...
        if (!isAdmin) {
            return sock.sendMessage(jid, { text: 'â›” Only admins can manage group backups.', contextInfo }, { quoted: message });
        }
            ...
            try {
                ...
                if (!files.length) {
                    return sock.sendMessage(jid, {
                        text: 'ðŸ“¦ *No backups found.*\n\nCreate one with `.backupgroup <name>`',
                        contextInfo
                    }, { quoted: message });
                }
 
                const list = files.map((f, i) => {
                    return `${i + 1}. ðŸ“¦ *${f.name}*\n   _${f.date.toLocaleDateString()} ${f.date.toLocaleTimeString()}_`;
                }).join('\n\n');
 
                return sock.sendMessage(jid, {
                    text: `ðŸ“¦ *Group Backups*\n\n${list}\n\n_Restore: \`.restoregroup <name>\`_`,
                    contextInfo
                }, { quoted: message });
            } catch {
                return sock.sendMessage(jid, { text: 'ðŸ“¦ No backups found.', contextInfo }, { quoted: message });
            }
        }
        ...
        if (['backupgroup', 'groupbackup'].includes(rawCmd)) {
            ...
            try {
                ...
                return sock.sendMessage(jid, {
                    text: `âœ… *Group backup created!*\n\nðŸ“¦ *Name:* ${backupName}\nðŸ‘¥ *Members:* ${backup.memberCount}\nðŸ‘‘ *Admins:* ${backup.admins.length}\nðŸ“ *Description:* ${backup.groupDesc ? 'Yes' : 'None'}\nðŸ‘‹ *Welcome settings:* ${backup.welcomeSettings ? 'Yes' : 'None'}\nðŸ§© *Quiz settings:* ${backup.quizSettings ? 'Yes' : 'None'}\n\n_Restore later: \`.restoregroup ${backupName}\`_`,
                    contextInfo
                }, { quoted: message });
            } catch (err) {
                return sock.sendMessage(jid, { text: `âŒ Backup failed: ${err.message}`, contextInfo }, { quoted: message });
            }
        }
        ...
        if (rawCmd === 'restoregroup') {
            ...
            if (!fs.existsSync(filePath)) {
                return sock.sendMessage(jid, {
                    text: `âŒ Backup "${backupName}" not found.\n\nUse \`.listbackups\` to see available backups.`,
                    contextInfo
                }, { quoted: message });
            }
            ...
            try {
                ...
                if (backup.groupDesc) {
                    try {
                        ...
                        restored.push('âœ… Description');
                    } catch { restored.push('âŒ Description (no permission)'); }
                }
