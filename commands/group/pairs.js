...
module.exports = {
    ...
    run: async (sock, message, args, { from, jid, prefix, contextInfo, mentionedJid }) => {
        ...
        if (cmd === 'divorce') {
            ...
            if (!key) return sock.sendMessage(jid, { text: 'âŒ You are not paired with anyone.', contextInfo }, { quoted: message });
            delete pairs[chatKey][key];
            savePairs(pairs);
            return sock.sendMessage(jid, { text: 'ðŸ’” Divorce processed. You are now single.', contextInfo }, { quoted: message });
        }
 
        const p1 = mentions[0] || from;
        const p2 = mentions[1];
        if (!p2) {
            return sock.sendMessage(jid, { text: 'ðŸ’• Mention two people to pair them!\nExample: .pair @person1 @person2', contextInfo }, { quoted: message });
        }
        if (!pairs[chatKey]) pairs[chatKey] = {};
        const alreadyPaired = Object.keys(pairs[chatKey]).find(k => k.includes(p1) || k.includes(p2));
        if (alreadyPaired) return sock.sendMessage(jid, { text: 'âŒ One of them is already paired! Use .divorce first.', contextInfo }, { quoted: message });
        ...
        await sock.sendMessage(jid, {
            text: `ðŸ’ *Congratulations!*\n\nðŸ’• @${n1} and @${n2} are now paired!\n\n_May your bond last forever ðŸ’–_`,
            mentions: [p1, p2],
            contextInfo
        }, { quoted: message });
    }
