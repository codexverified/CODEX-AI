...
module.exports = {
    ...
    run: async (sock, message, args, ctx) => {
        ...
        if (!isAudio) {
            return sock.sendMessage(jid, {
                text: 'âŒ Please reply to a *voice message* or audio file.',
                contextInfo
            }, { quoted: message });
        }
 
        await sock.sendMessage(jid, { text: 'ðŸŽ¤ Transcribing voice message...', contextInfo }, { quoted: message });
        ...
        try {
            ...
            await sock.sendMessage(jid, {
                text: `ðŸŽ¤ *Voice Message Info*\n\nâ±ï¸ *Duration:* ${durationStr}\nðŸ“¦ *Size:* ${(fileSize / 1024).toFixed(1)} KB\nðŸ’¬ *Est. words:* ~${estimatedWords}\n\n_Full speech-to-text requires a cloud STT API (Google/Whisper). Set up SPEECH_API_KEY in config for full transcription._\n\nðŸ’¡ *Tip:* You can use \`.tts <text>\` to convert text back to voice!`,
                contextInfo
            }, { quoted: message });
 
        } catch (err) {
            await sock.sendMessage(jid, {
                text: 'âŒ Failed to process voice message. Make sure you replied to a voice note.',
                contextInfo
            }, { quoted: message });
        }
    }
