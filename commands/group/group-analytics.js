...
module.exports = {
    ...
    category: 'group',
    description: 'View group chat analytics â€” top members, peak hours, activity stats',
    usage: '.analytics',
    permission: 'public',
    group: true,
    private: false,
    ...
    run: async (sock, message, args, ctx) => {
        ...
        if (rawCmd === 'topusers') {
            ...
            let list = sorted.map(([user, info], i) => {
                ...
                return `${i + 1}. @${num} â€” ${info.count} msgs ${bar}`;
            }).join('\n');
        ...
        const top5 = sorted.slice(0, 5).map(([user, info], i) => {
            const num = user.split('@')[0];
            return `${i + 1}. @${num} â€” ${info.count} msgs`;
        }).join('\n');
 
        const mentions = sorted.slice(0, 5).map(([user]) => user);
 
        return sock.sendMessage(jid, {
            text: `ðŸ“Š *Group Analytics*\n\nâ±ï¸ *Tracking since:* ${duration} ago\nðŸ“¨ *Total messages:* ${data.totalMessages}\nðŸ‘¥ *Active members:* ${memberCount}\nðŸ“ˆ *Avg per member:* ${avgPerMember}\nðŸ• *Peak hour:* ${String(peakHour).padStart(2, '0')}:00\n\nðŸ† *Top 5 Members:*\n${top5}\n\n_Use \`.topusers\` for full leaderboard_\n_Use \`.peakhours\` for hourly chart_`,
            mentions,
            contextInfo
        }, { quoted: message });
    }
