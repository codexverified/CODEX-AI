    ...
    run: async (sock, message, args, ctx) => {
        ...
        if (!args.length) {
            return sock.sendMessage(jid, {
                text: `âŒ *Please provide a message to send.*\n\nUsage: \`.dmall <your message>\``,
                contextInfo
            }, { quoted: message });
        }
        ...
        if (!meta) {
            try {
                ...
                return sock.sendMessage(jid, {
                    text: `âŒ Could not fetch group members: ${e.message}`,
                    contextInfo
                }, { quoted: message });
            }
        }
        ...
        if (!participants.length) {
            return sock.sendMessage(jid, {
                text: `âŒ No members found in this group.`,
                contextInfo
            }, { quoted: message });
        }
 
        await sock.sendMessage(jid, {
            text: `ðŸ“¨ *Broadcasting to ${participants.length} members...*`,
            contextInfo
        }, { quoted: message });
