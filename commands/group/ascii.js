    ...
    run: async (sock, message, args, ctx) => {
        ...
        if (!args.length) {
            return sock.sendMessage(jid, {
                text: `âŒ Provide text.\n\nUsage: \`.ascii Hello\`\n\nAvailable fonts:\n${FONTS.map(f => `â€¢ ${f}`).join('\n')}\n\nWith font: \`.ascii Hello --font Slant\``,
                contextInfo
            }, { quoted: message });
        }
        ...
        if (!text) {
            return sock.sendMessage(jid, { text: 'âŒ Provide text before the --font option.', contextInfo }, { quoted: message });
        }
 
        if (text.length > 30) {
            return sock.sendMessage(jid, { text: 'âŒ Text too long â€” max 30 characters for ASCII art.', contextInfo }, { quoted: message });
        }
 
        figlet.text(text, { font }, async (err, result) => {
            if (err || !result) {
                return sock.sendMessage(jid, { text: 'âŒ Failed to generate ASCII art.', contextInfo }, { quoted: message });
            }
