...
module.exports = {
    ...
    run: async (sock, message, args, { sender, groupId, prefix, isAdmin, contextInfo }) => {
        ...
        if (cmd === 'rules') {
            ...
            if (!text) {
                return sock.sendMessage(groupId, { text: 'ðŸ“‹ No rules set for this group yet.\nAdmin can use .setrules to add rules.', contextInfo }, { quoted: message });
            }
        ...
        if (!isAdmin) {
            return sock.sendMessage(sender, { text: 'âŒ Only admins can set rules.', contextInfo }, { quoted: message });
        }
 
        if (cmd === 'setrules') {
            const text = args.join(' ');
            if (!text) return sock.sendMessage(sender, { text: 'ðŸ“‹ Usage: .setrules <rules text>', contextInfo }, { quoted: message });
            rules[groupId] = text;
            saveRules(rules);
            return sock.sendMessage(groupId, { text: `âœ… Group rules updated!\n\nðŸ“‹ ${text}`, contextInfo }, { quoted: message });
        }
 
        if (cmd === 'delrules') {
            delete rules[groupId];
            saveRules(rules);
            return sock.sendMessage(groupId, { text: 'ðŸ—‘ï¸ Group rules deleted.', contextInfo }, { quoted: message });
        }
    }
};
