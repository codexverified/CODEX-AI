...
const { jidNormalizedUser, downloadMediaMessage } = require('../../lib/baileys');
    ...
    run: async (sock, message, args, ctx) => {
        ...
        if (['vcfread', 'readvcf', 'vcfview'].includes(rawCmd)) {
            ...
            try {
                ...
                try {
                    ...
                    return sock.sendMessage(jid, {
                        text: 'âŒ Could not download the VCF file. Try sending it again and replying with `.vcfread`.',
                        contextInfo
                    }, { quoted: message });
                }
            }
            ...
            if (!cards.length) {
                return sock.sendMessage(jid, { text: 'âŒ No contacts found in this file.', contextInfo }, { quoted: message });
            }
            ...
            for (let page = 0; page < totalPages; page++) {
                ...
                const header = totalPages > 1
                    ? `ðŸ“‡ *VCF Contents* â€” Page ${page + 1}/${totalPages} (${contacts.length} contacts)\n\n`
                    : `ðŸ“‡ *VCF Contents* â€” ${contacts.length} contact(s)\n\n`;
        ...
        if (rawCmd === 'vcfnumber') {
            ...
            for (const raw of numbers) {
                ...
                report.push(`âœ… +${phone} â†’ *${name}* (${source})`);
            }
            ...
            await sock.sendMessage(jid, {
                ...
                caption: `ðŸ“‡ *VCF Generated â€” ${numbers.length} contact(s)*\n\n${report.join('\n')}`
            }, { quoted: message });
            return;
        }
        ...
        if (rawCmd === 'vcfgroup' || (rawCmd === 'vcfgen' && isGroup && !args.length)) {
            ...
            if (!participants.length) {
                return sock.sendMessage(jid, { text: 'âŒ Could not fetch group members.', contextInfo }, { quoted: message });
            }
            ...
            await sock.sendMessage(jid, {
                text: `â³ Generating VCF for *${participants.length}* members...\nðŸ“Š Cache: LID=${lidMapSize} | Names=${pushCacheSize} | Contacts=${lidJidSize}`,
                contextInfo
            }, { quoted: message });
            ...
            if (!vcards.length) {
                return sock.sendMessage(jid, {
                    text: [
                        'âŒ Could not resolve any phone numbers from group members.',
                        '',
            ...
            await sock.sendMessage(jid, {
                ...
                caption: [
                    `ðŸ“‡ *VCF Generated â€” ${groupName}*`,
                    ``,
                    `ðŸ‘¥ Total contacts: *${vcards.length}*`,
                    `ðŸ“› With names: *${named}*`,
                    `ðŸ“ž Phone only: *${phoneOnly}*`,
                    skipped > 0 ? `âš ï¸ Unresolved: *${skipped}* (not yet seen by bot)` : '',
                    ``,
        ...
        if (rawCmd === 'vcfgen' || rawCmd === 'savecontacts' || rawCmd === 'exportvcf' || rawCmd === 'contactsave') {
            ...
            if (mentioned.length) {
                await sock.sendMessage(jid, {
                    text: `â³ Looking up *${mentioned.length}* mentioned user(s)...`,
                    contextInfo
                }, { quoted: message });
                ...
                if (!vcards.length) {
                    return sock.sendMessage(jid, { text: 'âŒ No valid contacts found in mentions.', contextInfo }, { quoted: message });
                }
                ...
                await sock.sendMessage(jid, {
                    ...
                    caption: `ðŸ“‡ *VCF Generated â€” ${vcards.length} contact(s)*\n\n${report.join('\n')}`
                }, { quoted: message });
                return;
            }
            ...
            return sock.sendMessage(jid, {
                text: [
                    `ðŸ“‡ *VCF Contact Generator*`,
                    ``,
                    `Generate .vcf contact files with real names.`,
                    ``,
                    `*Commands:*`,
                    `â€¢ \`.vcfgen\` â€” Export all group members as VCF`,
                    `â€¢ \`.vcfgen @user1 @user2\` â€” VCF for tagged users`,
                    `â€¢ \`.vcfnumber 2547XXXXXXXX\` â€” VCF from phone number(s)`,
                    `â€¢ \`.vcfgroup\` â€” Full group export with name lookup`,
                    `â€¢ \`.vcfread\` â€” Reply to a .vcf file to view its contents`,
                    ``,
                    ...
                    ``,
                    `_Names are cached from messages â€” more activity = more names._`,
                ].join('\n'),
                contextInfo
            }, { quoted: message });
        }
