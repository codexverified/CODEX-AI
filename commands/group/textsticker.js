...
module.exports = {
    ...
    run: async (sock, message, args, ctx) => {
        ...
        if (!args.length) {
            const styleList = Object.keys(styles).map(s => `â€¢ ${s}`).join('\n');
            return sock.sendMessage(jid, {
                text: `ðŸŽ¨ *Text Sticker Maker*\n\nCreate beautiful text stickers!\n\n*Usage:*\n\`.textsticker <text>\`\n\`.textsticker neon Hello World\`\n\n*Available Styles:*\n${styleList}`,
                contextInfo
            }, { quoted: message });
        }
        ...
        if (!text.trim()) {
            return sock.sendMessage(jid, { text: 'âŒ Please provide some text.', contextInfo }, { quoted: message });
        }
 
        if (text.length > 100) {
            return sock.sendMessage(jid, { text: 'âŒ Text too long. Max 100 characters.', contextInfo }, { quoted: message });
        }
        ...
        try {
            ...
            await sock.sendMessage(jid, { text: 'âŒ Failed to create sticker. Try shorter text.', contextInfo }, { quoted: message });
        }
    }
};
