    ...
    run: async (sock, message, args, ctx) => {
        ...
        try {
            ...
            if (!url || !/^https?:\/\//.test(url)) {
                return reply('âœ³ï¸ Provide a valid URL starting with http:// or https://');
            }
            ...
            if (contentLength > 100 * 1024 * 1024) {
                return reply(`âŒ File too large: ${(contentLength / 1024 / 1024).toFixed(1)} MB`);
            }
 
            const contentType = res.headers['content-type'] || '';
 
            // Non-text/JSON â†’ send as document
            ...
            return reply(`âŒ Error fetching URL:\n${err.message}`);
        }
    },
};
