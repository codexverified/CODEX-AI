    ...
    run: async (sock, message, args, ctx) => {
        ...
        if (cmd === 'livescore') {
            try {
                ...
                if (!events.length) {
                    return send(`âš½ *Live Scores*\n\nNo live matches right now.\n\nðŸ”— https://www.livescore.com\nðŸ”— https://www.flashscore.com`);
                }
                const list = events.map(e => {
                    ...
                    return `âš½ *${hn}* ${hs}-${as} *${an}*  _${status}_`;
                }).join('\n');
                return send(`âš½ *Live Scores*\n\n${list}\n\n_Updated: ${new Date().toLocaleTimeString()}_`);
            } catch {
                return send(`âš½ *Live Scores*\n\nðŸ”— https://www.livescore.com\nðŸ”— https://www.flashscore.com\nðŸ”— https://www.sofascore.com`);
            }
        }
        ...
        if (cmd === 'sportnews') {
            try {
                ...
                const list = articles.map((a, i) =>
                    `*${i + 1}.* ${a.title}\n   ðŸ“° ${a.source?.name} | ${a.publishedAt?.split('T')[0]}`
                ).join('\n\n');
                return send(`ðŸ† *Sports News*\n\n${list}\n\nðŸ”— https://www.bbc.com/sport`);
            } catch {
                return send(`ðŸ† *Sports News*\n\nðŸ“° Latest coverage:\nâ€¢ https://www.bbc.com/sport\nâ€¢ https://www.espn.com\nâ€¢ https://www.goal.com`);
            }
        }
        ...
        if (cmd === 'standings') {
            ...
            try {
                ...
                return send(`ðŸ† *Standings: ${league}*\n\n${list}`);
            } catch {
                return send(
                    `ðŸ† *${league} Standings*\n\n` +
                    `ðŸ”— https://www.flashscore.com/football/\n` +
                    `ðŸ”— https://www.bbc.com/sport/football/tables`
                );
            }
        }
        ...
        if (cmd === 'topscorers') {
            ...
            try {
                ...
                const list = leaders.map((l, i) =>
                    `*${i + 1}.* ${l.athlete?.displayName || '?'} (${l.team?.abbreviation || '?'}) â€” âš½ ${l.value} goals`
                ).join('\n');
                return send(`âš½ *Top Scorers: ${league}*\n\n${list}`);
            } catch {
                return send(
                    `âš½ *Top Scorers: ${league}*\n\n` +
                    `ðŸ”— https://www.whoscored.com\n` +
                    `ðŸ”— https://www.bbc.com/sport/football/premier-league/top-scorers`
                );
            }
        }
        ...
        if (cmd === 'upcomingmatches') {
            ...
            try {
                ...
                const list = upcoming.map(e => {
                    ...
                    return `ðŸ“… ${d.toDateString()} | ${home} vs ${away}`;
                }).join('\n');
                return send(`ðŸ“… *Upcoming: ${found.team.displayName}*\n\n${list}`);
            } catch {
                return send(`ðŸ“… *Upcoming Matches: ${team}*\n\nðŸ”— https://www.espn.com/soccer/search/_/q/${encodeURIComponent(team)}`);
            }
        }
        ...
        if (cmd === 'surebet') {
            ...
            return send(
                `ðŸ’° *Sure Bets: ${league}*\n\n` +
                `ðŸ“Š For today's predictions:\n` +
                `ðŸ”— https://www.betensured.com\n` +
                `ðŸ”— https://www.soccervista.com\n` +
                `ðŸ”— https://www.forebet.com\n\n` +
                `âš ï¸ _Bet responsibly. Never wager more than you can afford to lose._`
            );
        }
        ...
        if (cmd === 'gamehistory') {
            ...
            try {
                ...
                const list = past.map(e => {
                    ...
                    const hs   = home?.score ?? '-';
                    const as   = away?.score ?? '-';
                    return `ðŸ“… ${d.toDateString()}\n   ${hn} ${hs}-${as} ${an}`;
                }).join('\n\n');
                return send(`ðŸ“‹ *Match History: ${found.team.displayName}*\n\n${list}`);
            } catch {
                return send(`ðŸ“‹ *Match History: ${team}*\n\nðŸ”— https://www.espn.com/soccer/search/_/q/${encodeURIComponent(team)}`);
            }
        }
    }
};
