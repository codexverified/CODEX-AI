...
module.exports = {
    ...
    category: 'admin',
    description: 'View admin action audit log â€” tracks kicks, promotes, demotes, settings changes',
    ...
    run: async (sock, message, args, ctx) => {
        ...
        if (!isAdmin) {
            return sock.sendMessage(jid, { text: 'â›” Only admins can view the audit log.', contextInfo }, { quoted: message });
        }
        ...
        if (!logs.length) {
            return sock.sendMessage(jid, {
                text: 'ðŸ“‹ *Audit Log*\n\nNo actions recorded yet.\n\nThe bot tracks:\nâ€¢ Kicks & removals\nâ€¢ Promotes & demotes\nâ€¢ Group settings changes\nâ€¢ Ban/unban actions\nâ€¢ Welcome/goodbye toggles',
                contextInfo
            }, { quoted: message });
        }
        ...
        if (mentioned.length) {
            ...
            if (!filtered.length) {
                return sock.sendMessage(jid, {
                    text: `ðŸ“‹ No audit entries found for @${target.split('@')[0]}.`,
                    mentions: [target],
                    contextInfo
                }, { quoted: message });
            }
        ...
        const actionIcons = {
            kick: 'ðŸš«', promote: 'â¬†ï¸', demote: 'â¬‡ï¸', ban: 'ðŸ”¨',
            unban: 'âœ…', mute: 'ðŸ”‡', unmute: 'ðŸ”Š', settings: 'âš™ï¸',
            welcome: 'ðŸ‘‹', antilink: 'ðŸ”—', warn: 'âš ï¸', default: 'ðŸ“'
        };
        ...
        const list = recent.map(l => {
            ...
            const targetStr = l.targetNum ? ` â†’ @${l.targetNum}` : '';
