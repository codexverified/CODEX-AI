...
module.exports = {
    ...
    run: async (sock, message, args, ctx) => {
        ...
        const adminCheck = () => {
            if (!isAdmin) { send(theme.admin || 'Ã¢â€ºâ€ Only admins can use this command.'); return false; }
            return true;
        };
        const botAdminCheck = () => {
            if (!isBotAdmin) { send(theme.botAdmin || 'Ã¢â€ºâ€ I need to be an admin.'); return false; }
            return true;
        };
        ...
        if (cmd === 'link') {
            ...
            try {
                ...
                return send(`Ã°Å¸â€â€” *Group Invite Link*\n\nhttps://chat.whatsapp.com/${link}\n\n_Use .resetlink to revoke this link_`);
            } catch { return send('Ã¢ÂÅ’ Failed to get invite link. Make sure I am an admin.'); }
        }
        ...
        if (cmd === 'resetlink') {
            ...
            try {
                ...
                return send('Ã°Å¸â€â€ž *Invite link revoked!* A new link has been generated.');
            } catch { return send('Ã¢ÂÅ’ Failed to revoke invite link.'); }
        }
 
        if (cmd === 'groupname') {
            if (!adminCheck() || !botAdminCheck()) return;
            const name = args.join(' ').trim();
            if (!name) return send('Ã¢ÂÅ’ *Usage:* `.groupname <new name>`');
            try {
                await sock.groupUpdateSubject(jid, name);
                return send(`Ã¢Å“â€¦ Group name changed to *${name}*`);
            } catch { return send('Ã¢ÂÅ’ Failed to update group name.'); }
        }
 
        if (cmd === 'gcdesc') {
            if (!adminCheck() || !botAdminCheck()) return;
            const desc = args.join(' ').trim();
            if (!desc) return send('Ã¢ÂÅ’ *Usage:* `.gcdesc <new description>`');
            try {
                await sock.groupUpdateDescription(jid, desc);
                return send('Ã¢Å“â€¦ Group description updated!');
            } catch { return send('Ã¢ÂÅ’ Failed to update description.'); }
        }
        ...
        if (cmd === 'gcpp') {
            ...
            if (!imgMsg) return send('Ã¢ÂÅ’ Reply to or send an image with `.gcpp`');
            try {
                const { dlBuffer } = require('../../lib/dlmedia');
                const buf = await dlBuffer(imgMsg, 'image');
                await sock.updateProfilePicture(jid, buf);
                return send('Ã¢Å“â€¦ Group photo updated!');
            } catch { return send('Ã¢ÂÅ’ Failed to update group photo.'); }
        }
 
        if (cmd === 'getgcpp') {
            try {
                const ppUrl = await sock.profilePictureUrl(jid, 'image');
                await sock.sendMessage(jid, { image: { url: ppUrl }, caption: fmt('Ã°Å¸â€“Â¼Ã¯Â¸Â *Group Profile Picture*'), contextInfo }, { quoted: message });
            } catch { return send('Ã¢ÂÅ’ Could not fetch group photo.'); }
            return;
        }
        ...
        if (cmd === 'getlid') {
            ...
            if (!target) return send('Ã¢ÂÅ’ Reply to or mention a user.\n\n*Usage:* `.getlid @user`');
            const p = participants.find(x => x.id === target || x.lid === target);
            return send(`Ã°Å¸â€ â€ *LID Info*\n\nÃ°Å¸â€˜Â¤ Phone: @${target.split('@')[0]}\nÃ°Å¸â€â€“ LID: ${p?.lid || 'N/A'}`, { mentions: [target] });
        }
 
        if (cmd === 'add') {
            if (!adminCheck() || !botAdminCheck()) return;
            const nums = args.join(' ').replace(/[^0-9,\s]/g, '').split(/[\s,]+/).filter(Boolean);
            if (!nums.length) return send('Ã¢ÂÅ’ *Usage:* `.add <number(s)>`\n\nExample: `.add 254712345678`');
        ...
        if (cmd === 'del') {
            ...
            if (!key) return send('Ã¢ÂÅ’ Reply to the message you want to delete.');
            try {
                await sock.sendMessage(jid, { delete: { remoteJid: jid, fromMe: false, id: key, participant } });
                return send('Ã°Å¸â€”â€˜Ã¯Â¸Â Message deleted.');
            } catch { return send('Ã¢ÂÅ’ Failed to delete message. Make sure I am an admin.'); }
        }
        ...
        if (cmd === 'mute') {
            ...
            try {
                ...
                return send('Ã°Å¸â€â€¡ *Group muted!* Only admins can send messages.');
            } catch { return send('Ã¢ÂÅ’ Failed to mute group.'); }
        }
        ...
        if (cmd === 'unmute' || (cmd === 'open' && args.length === 0)) {
            ...
            try {
                ...
                return send('Ã°Å¸â€Å  *Group unmuted!* Everyone can send messages.');
            } catch { return send('Ã¢ÂÅ’ Failed to unmute group.'); }
        }
        ...
        if (cmd === 'disapp') {
            ...
            try {
                ...
                return send(`Ã¢ÂÂ³ Disappearing messages set to: *${label}*`);
            } catch { return send('Ã¢ÂÅ’ Failed to set disappearing messages.'); }
        }
        ...
        if (cmd === 'groupsettings') {
            ...
            return send(
                `Ã¢Å¡â„¢Ã¯Â¸Â *Group Settings*\n\n` +
                `Ã°Å¸â€˜Â¥ Members: *${p.length}*\n` +
                `Ã°Å¸â€˜â€˜ Admins: *${admins}*\n` +
                `Ã°Å¸â€â€™ Locked: *${groupMetadata?.announce ? 'Yes' : 'No'}*\n` +
                `Ã¢ÂÂ³ Disappear: *${label}*\n` +
                `Ã°Å¸â€œÂ Description: ${groupMetadata?.desc ? groupMetadata.desc.slice(0, 80) : 'None'}`
            );
        }
        ...
        if (cmd === 'killgc') {
            ...
            await send('Ã°Å¸â€™â‚¬ *Goodbye!* The bot is leaving this group...');
        ...
        if (cmd === 'newgroup') {
            if (!isOwner) return send('Ã¢â€ºâ€ Owner only.');
            const name = args.join(' ').trim();
            if (!name) return send('Ã¢ÂÅ’ *Usage:* `.newgroup <group name>`');
            try {
                const created = await sock.groupCreate(name, []);
                return send(`Ã¢Å“â€¦ Group *${name}* created!\nJID: ${created.id}`);
            } catch { return send('Ã¢ÂÅ’ Failed to create group.'); }
        }
 
        if (cmd === 'everyone') {
            if (!adminCheck()) return;
            const text = args.join(' ').trim() || 'Ã°Å¸â€˜â€¹ Attention everyone!';
            const mentions = participants.map(p => p.id || p.jid || p.phoneNumber).filter(Boolean);
            const names    = mentions.map(j => `@${j.split('@')[0]}`).join(' ');
            return sock.sendMessage(jid, { text: fmt(`Ã°Å¸â€œÂ¢ ${text}\n\n${names}`), mentions, contextInfo }, { quoted: message });
        }
 
        if (cmd === 'tagadmins') {
            if (!adminCheck()) return;
            const admins  = participants.filter(p => p.admin).map(p => p.id || p.jid || p.phoneNumber).filter(Boolean);
            if (!admins.length) return send('Ã¢ÂÅ’ No admins found.');
            const text = args.join(' ').trim() || 'Ã°Å¸â€˜â€˜ Admins, your attention please!';
        ...
        if (cmd === 'listrequests') {
            ...
            try {
                ...
                if (!reqs?.length) return send('Ã°Å¸â€œâ€¹ *Join Requests*\n\nNo pending join requests.');
                const list = reqs.slice(0, 20).map((r, i) => `*${i + 1}.* +${r.jid?.split('@')[0]}`).join('\n');
                return send(`Ã°Å¸â€œâ€¹ *Join Requests (${reqs.length})*\n\n${list}\n\nUse \`.accept all\` or \`.reject all\``);
            } catch { return send('Ã¢ÂÅ’ Failed to fetch join requests.'); }
        }
        ...
        if (cmd === 'accept') {
            ...
            try {
                const reqs = await sock.groupRequestParticipantsList(jid);
                if (!reqs?.length) return send('Ã°Å¸â€œâ€¹ No pending join requests.');
                const sub = args[0]?.toLowerCase();
                if (sub === 'all') {
                    await sock.groupRequestParticipantsUpdate(jid, reqs.map(r => r.jid), 'approve');
                    return send(`Ã¢Å“â€¦ Approved all *${reqs.length}* pending requests.`);
                }
                const target = mentionedJid?.[0] || (args[0] ? `${args[0].replace(/\D/g, '')}@s.whatsapp.net` : null);
                if (!target) return send('Ã¢ÂÅ’ Mention or provide number, or use `.accept all`');
                await sock.groupRequestParticipantsUpdate(jid, [target], 'approve');
                return send(`Ã¢Å“â€¦ Approved @${target.split('@')[0]}'s join request.`, { mentions: [target] });
            } catch { return send('Ã¢ÂÅ’ Failed to accept request.'); }
        }
        ...
        if (cmd === 'acceptall') {
            ...
            try {
                ...
                await sock.groupRequestParticipantsUpdate(jid, reqs.map(r => r.jid), 'approve');
                return send(`Ã¢Å“â€¦ Approved all *${reqs.length}* pending requests.`);
            } catch { return send('Ã¢ÂÅ’ Failed to accept all requests.'); }
        }
        ...
        if (cmd === 'reject') {
            ...
            try {
                ...
                if (sub === 'all') {
                    ...
                    return send(`Ã¢ÂÅ’ Rejected all *${reqs.length}* pending requests.`);
                }
                const target = mentionedJid?.[0] || (args[0] ? `${args[0].replace(/\D/g, '')}@s.whatsapp.net` : null);
                if (!target) return send('Ã¢ÂÅ’ Mention a user or use `.reject all`');
                await sock.groupRequestParticipantsUpdate(jid, [target], 'reject');
                return send(`Ã¢ÂÅ’ Rejected @${target.split('@')[0]}'s join request.`, { mentions: [target] });
            } catch { return send('Ã¢ÂÅ’ Failed to reject request.'); }
        }
        ...
        if (cmd === 'rejectall') {
            ...
            try {
                ...
                await sock.groupRequestParticipantsUpdate(jid, reqs.map(r => r.jid), 'reject');
                return send(`Ã¢ÂÅ’ Rejected all *${reqs.length}* pending requests.`);
            } catch { return send('Ã¢ÂÅ’ Failed to reject all requests.'); }
        }
 
        if (cmd === 'online') {
            const text = participants.map((p, i) => `*${i + 1}.* @${p.id.split('@')[0]}`).join('\n');
            return sock.sendMessage(jid, {
                text: fmt(`Ã°Å¸â€˜Â¥ *Group Members (${participants.length})*\n\n${text}`),
        ...
        if (cmd === 'left') {
            ...
            if (!lefts.length) return send('Ã°Å¸â€œÂ¤ *Left Members*\n\nNo one has left this group recently.');
            const list = lefts.map((j, i) => `*${i + 1}.* +${j.split('@')[0]}`).join('\n');
            return send(`Ã°Å¸â€œÂ¤ *Recently Left Members (${lefts.length})*\n\n${list}`);
        }
        ...
        if (cmd === 'vcf') {
            ...
            await sock.sendMessage(jid, {
                ...
                caption:  fmt(`Ã°Å¸â€œâ€¡ Exported *${participants.length}* contacts`)
            }, { quoted: message });
            return;
        }
        ...
        if (cmd === 'badwords') {
            ...
            if (!words.length) return send('Ã°Å¸Å¡Â« *Bad Words Filter*\n\nNo bad words set.\n\nUse `.setantibad <word1, word2>` to add words.');
            return send(`Ã°Å¸Å¡Â« *Bad Words (${words.length})*\n\n${words.map(w => `Ã¢â‚¬Â¢ ${w}`).join('\n')}`);
        }
 
        if (cmd === 'setantibad') {
            if (!adminCheck()) return;
            const words = args.join(' ').split(/[,\s]+/).map(w => w.trim().toLowerCase()).filter(Boolean);
            if (!words.length) return send('Ã¢ÂÅ’ *Usage:* `.setantibad <word1, word2, ...>`');
            const existing = loadBadWords(jid);
            const merged   = [...new Set([...existing, ...words])];
            saveBadWords(jid, merged);
            return send(`Ã¢Å“â€¦ Added *${words.length}* word(s) to bad word filter.\nTotal: *${merged.length}* words`);
        }
        ...
        if (cmd === 'setantilink') {
            ...
            if (sub === 'on' || sub === 'enable') {
                ...
                return send('Ã°Å¸â€â€” *Anti-Link ON* Ã¢â‚¬â€ Links will be deleted and sender warned.');
            }
            global.antilinkGroups.delete(jid);
            return send('Ã°Å¸â€â€” *Anti-Link OFF*');
        }
        ...
        if (cmd === 'antigroupmention') {
            ...
            const sub = (args[0] || 'on').toLowerCase();
            if (sub === 'on') { global.antiGcMention.add(jid); return send('Ã°Å¸â€â€¢ *Anti-Group Mention ON*'); }
            global.antiGcMention.delete(jid);
            return send('Ã°Å¸â€â€¢ *Anti-Group Mention OFF*');
        }
        ...
        if (cmd === 'antipromote') {
            ...
            const sub = (args[0] || 'on').toLowerCase();
            if (sub === 'on') { global.antiPromote.add(jid); return send('Ã¢Â¬â€ Ã¯Â¸Â *Anti-Promote ON* Ã¢â‚¬â€ Non-admin promotions will be reversed.'); }
            global.antiPromote.delete(jid);
            return send('Ã¢Â¬â€ Ã¯Â¸Â *Anti-Promote OFF*');
        }
 
        if (cmd === 'antibadwarn') {
            if (!adminCheck()) return;
            return send('Ã¢Å¡Â Ã¯Â¸Â *Anti-Bad Word Warn*\n\nMembers who use bad words are automatically warned.\n\nManage words with `.setantibad <words>` and `.badwords`');
        }
 
        if (cmd === 'antilinkwarn') {
            if (!adminCheck()) return;
            return send('Ã¢Å¡Â Ã¯Â¸Â *Anti-Link Warn*\n\nMembers who post links get warned.\n\nToggle with `.setantilink on/off`');
        }
 
        if (cmd === 'setantigcmentionwarnlimit') {
            if (!adminCheck()) return;
            const limit = parseInt(args[0]);
            if (!limit || limit < 1) return send('Ã¢ÂÅ’ *Usage:* `.setantigcmentionwarnlimit <number>`\n\nExample: `.setantigcmentionwarnlimit 3`');
            if (!global.antiGcMentionLimits) global.antiGcMentionLimits = new Map();
            global.antiGcMentionLimits.set(jid, limit);
            return send(`Ã¢Å“â€¦ Anti-group-mention warn limit set to *${limit}* warnings.`);
        }
        ...
        if (cmd === 'setgroupevents') {
            ...
            if (sub === 'off') { global.groupEventsDisabled.add(jid); return send('Ã°Å¸â€œÂ¢ *Group Events OFF* Ã¢â‚¬â€ Join/leave messages will be suppressed.'); }
            global.groupEventsDisabled.delete(jid);
            return send('Ã°Å¸â€œÂ¢ *Group Events ON* Ã¢â‚¬â€ Join/leave messages will be sent.');
        }
 
        if (cmd === 'welcomemessage') {
            if (!global.welcomeSettings) return send('Ã¢ÂÅ’ Welcome plugin not loaded.');
            const s = global.welcomeSettings.get(jid) || {};
            const status = s.welcome ? 'Ã¢Å“â€¦ ON' : 'Ã¢ÂÅ’ OFF';
            const msg    = s.customWelcome || '(default)';
            return send(`Ã°Å¸â€˜â€¹ *Welcome Message*\n\nStatus: *${status}*\nMessage: ${msg}\n\nEdit with \`.setwelcome <message>\``);
        }
 
        if (cmd === 'goodbyemessage') {
            if (!global.welcomeSettings) return send('Ã¢ÂÅ’ Welcome plugin not loaded.');
            const s = global.welcomeSettings.get(jid) || {};
            const status = s.goodbye ? 'Ã¢Å“â€¦ ON' : 'Ã¢ÂÅ’ OFF';
            const msg    = s.customGoodbye || '(default)';
            return send(`Ã°Å¸â€˜â€¹ *Goodbye Message*\n\nStatus: *${status}*\nMessage: ${msg}\n\nEdit with \`.setgoodbye <message>\``);
        }
        ...
        if (cmd === 'resetgroup') {
            ...
            try {
                ...
                return send('Ã°Å¸â€â€ž Group settings reset to defaults.');
            } catch { return send('Ã¢ÂÅ’ Failed to reset group settings.'); }
        }
 
        if (cmd === 'menus') {
            const menuPlugin = require('./menu');
            if (menuPlugin?.run) return menuPlugin.run(sock, message, args, ctx);
            return send('Ã¢ÂÅ’ Menu plugin not found.');
        }
        ...
        if (cmd === 'list') {
            ...
            for (const chunk of chunks) {
                await sock.sendMessage(jid, {
                    text: fmt(`Ã°Å¸â€œâ€¹ *Command List*\n\n${chunk.map(c => `Ã¢â‚¬Â¢ .${c}`).join('\n')}`),
                    contextInfo
                }, { quoted: message });
            }
            return;
