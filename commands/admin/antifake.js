...
module.exports = {
    ...
    async run(sock, message, args, ctx) {
        ...
        if (!sub) {
            return reply(
                `*${botName}* â€” Anti Fake Numbers\n\n` +
                `Status: ${cfg.enabled ? 'âœ… ON' : 'âŒ OFF'}\n` +
                `Action: *${cfg.action}*\n\n` +
                `*Commands:*\n` +
                `â€¢ \`.antifake on\` â€” enable\n` +
                `â€¢ \`.antifake off\` â€” disable\n` +
                `â€¢ \`.antifake action kick\` â€” kick fakes (default)\n` +
                `â€¢ \`.antifake action warn\` â€” warn but keep them\n\n` +
            ...
            if (cfg.action === 'kick') {
                try {
                    ...
                    await sock.sendMessage(jid, {
                        text: `*${botName}*\n\nðŸš« Removed ${names} â€” unrecognized phone number(s).`,
                        mentions
                    });
                } catch { /* ignore */ }
                ...
                const names = fakeUsers.map(p => `@${p.split('@')[0]}`).join(', ');
                await sock.sendMessage(jid, {
                    text: `*${botName}*\n\nâš ï¸ Warning: ${names} appear to have unrecognized phone numbers.`,
                    mentions
                });
            }
        } catch { /* ignore */ }
