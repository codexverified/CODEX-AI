...
module.exports = {
    ...
    run: async (sock, message, args, ctx) => {
        ...
        if (!isAdmin) return reply(fmt('â›” Only admins can manage flood protection.'));
        ...
        if (rawCmd === 'floodoff' || (rawCmd === 'antiflood' && (args[0] || '').toLowerCase() === 'off')) {
            gs.enabled = false;
            settings.set(jid, gs);
            return reply(fmt('ðŸ›¡ï¸ *Anti-Flood: OFF*'));
        }
 
        if (rawCmd === 'floodlimit') {
            const limit  = parseInt(args[0]);
            const window = parseInt(args[1]);
            if (!limit || limit < 2) return reply(fmt('âŒ Limit must be at least 2 messages.'));
            if (!window || window < 3) return reply(fmt('âŒ Window must be at least 3 seconds.'));
            gs.limit  = limit;
            gs.window = window;
            settings.set(jid, gs);
            return reply(fmt(`âœ… Flood limit set: *${limit} messages* per *${window}s*`));
        }
 
        // Default: show status
        const state = gs.enabled ? 'âœ… ON' : 'âŒ OFF';
        return reply(fmt(
            `ðŸ›¡ï¸ *Anti-Flood Status:* ${state}\n` +
            `Limit: *${gs.limit} msgs* / *${gs.window}s*\n\n` +
            `â€¢ \`.antiflood on\` â€” enable\n` +
            `â€¢ \`.antiflood off\` â€” disable\n` +
            `â€¢ \`.floodlimit 5 10\` â€” 5 msgs per 10s`
        ));
    }
};
