...
function getRandomChallenge() {
    const allChallenges = [
        ...triviaQuestions.map(c => ({ ...c, type: 'ðŸ§  Trivia' })),
        ...puzzles.map(c => ({ ...c, type: 'ðŸ§© Puzzle' })),
        ...wordGames.map(c => ({ ...c, type: 'ðŸ”¤ Word Game' })),
    ];
    ...
    run: async (sock, message, args, ctx) => {
        ...
        if (rawCmd === 'challengenow' || rawCmd === 'challenge') {
            ...
            return sock.sendMessage(jid, {
                text: `${challenge.type} *Daily Challenge!*\n\nâ“ ${challenge.q}\n\nðŸ’¡ Reply with your answer!\n\n_Answer: ||${challenge.a}||_\n_React with âœ… if you got it right!_`,
                contextInfo
            }, { quoted: message });
        }
 
        if (!isAdmin) {
            return sock.sendMessage(jid, { text: 'â›” Only admins can configure daily challenges.', contextInfo }, { quoted: message });
        }
        ...
        if (rawCmd === 'challengeoff') {
            ...
            return sock.sendMessage(jid, { text: 'âœ… *Daily challenges disabled.*', contextInfo }, { quoted: message });
        }
        ...
        if (rawCmd === 'setchallenge') {
            ...
            if (!/^\d{1,2}:\d{2}$/.test(time)) {
                return sock.sendMessage(jid, {
                    text: 'âŒ Invalid time. Use HH:MM format.\n\nExample: `.setchallenge 09:00`',
                    contextInfo
                }, { quoted: message });
            }
        ...
        if (rawCmd === 'dailychallenge') {
            ...
            const status = settings?.enabled ? `âœ… ON at ${settings.time}` : 'âŒ OFF';
            return sock.sendMessage(jid, {
                text: `ðŸŽ¯ *Daily Challenges*\n\n*Status:* ${status}\n\n*Commands:*\nâ€¢ \`.setchallenge 09:00\` â€” schedule daily\nâ€¢ \`.challengenow\` â€” instant challenge\nâ€¢ \`.challengeoff\` â€” disable\n\n*Types:* Trivia, Puzzles, Word Games`,
                contextInfo
            }, { quoted: message });
        }
    }
