...
module.exports = {
    ...
    run: async (sock, message, args, ctx) => {
        ...
        if (rawCmd === 'clearreminders') {
            ...
            return sock.sendMessage(jid, { text: `ðŸ—‘ï¸ *${userReminders.length} reminders cleared.*`, contextInfo }, { quoted: message });
        }
        ...
        if (rawCmd === 'stopremind') {
            ...
            if (!id || !reminders[id]) {
                return sock.sendMessage(jid, {
                    text: 'âŒ Provide a valid reminder ID.\n\nUse `.myreminders` to see your IDs.',
                    contextInfo
                }, { quoted: message });
            }
            if (reminders[id].sender !== sender) {
                return sock.sendMessage(jid, { text: 'âŒ You can only stop your own reminders.', contextInfo }, { quoted: message });
            }
            if (activeIntervals.has(id)) { clearInterval(activeIntervals.get(id)); activeIntervals.delete(id); }
            reminders[id].active = false;
            saveReminders(reminders);
            return sock.sendMessage(jid, { text: `âœ… *Reminder \`${id}\` stopped.*`, contextInfo }, { quoted: message });
        }
        ...
        if (rawCmd === 'myreminders') {
            ...
            return sock.sendMessage(jid, {
                text: `ðŸ“‹ *Your Recurring Reminders*\n\n${list}\n\n_Stop: \`.stopremind <id>\`_`,
                contextInfo
            }, { quoted: message });
        }
        ...
        if (['rremind', 'recurringremind', 'repeatremind'].includes(rawCmd)) {
            if (!args.length) {
                const intervalList = Object.keys(intervals).map(i => `â€¢ ${i}`).join('\n');
                return sock.sendMessage(jid, {
                    text: `ðŸ” *Recurring Reminders*\n\nSet reminders that repeat automatically!\n\n*Usage:*\n\`.rremind <interval> <message>\`\n\n*Intervals:*\n${intervalList}\n\n*Examples:*\n\`.rremind daily Take vitamins\`\n\`.rremind weekly Team standup\`\n\`.rremind 2h Drink water\`\n\n*Other commands:*\nâ€¢ \`.myreminders\` â€” list active\nâ€¢ \`.stopremind <id>\` â€” stop one\nâ€¢ \`.clearreminders\` â€” stop all`,
                    contextInfo
                }, { quoted: message });
            }
            ...
            if (!intervals[interval]) {
                return sock.sendMessage(jid, {
                    text: `âŒ Invalid interval. Choose from: ${Object.keys(intervals).join(', ')}`,
                    contextInfo
                }, { quoted: message });
            }
            ...
            if (!msg) {
                return sock.sendMessage(jid, { text: 'âŒ Please provide a reminder message.', contextInfo }, { quoted: message });
            }
 
            const userCount = Object.values(reminders).filter(r => r.sender === sender && r.active).length;
            if (userCount >= 10) {
                return sock.sendMessage(jid, { text: 'âŒ Max 10 active reminders. Delete some first with `.clearreminders`.', contextInfo }, { quoted: message });
            }
            ...
            return sock.sendMessage(jid, {
                text: `âœ… *Recurring reminder set!*\n\nðŸ”” *ID:* \`${id}\`\nðŸ“ *Message:* ${msg}\nðŸ” *Repeats:* Every ${interval}\n\n_Stop anytime: \`.stopremind ${id}\`_`,
                contextInfo
            }, { quoted: message });
        }
    }
