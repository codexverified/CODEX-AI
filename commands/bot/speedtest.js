...
module.exports = {
    ...
    run: async (sock, message, args, { sender, groupId, contextInfo }) => {
        ...
        await sock.sendMessage(chatId, { text: 'â³ Testing internet speed...', contextInfo }, { quoted: message });
        try {
            ...
            await sock.sendMessage(chatId, {
                text:
`ðŸŒ *Server Speed Test*
 
â¬‡ï¸ *Download:* ${mbps} Mbps
âš¡ *Latency:*   ${latency}ms
ðŸ“¦ *Data:*      ${(bytes / 1024 / 1024).toFixed(2)} MB in ${elapsed.toFixed(2)}s
ðŸ–¥ï¸ *Server:*   ${detectPlatform()} (${process.platform})
 
_Powered by Cloudflare â€¢ CODEX AI_`,
                contextInfo
            }, { quoted: message });
        } catch (e) {
            await sock.sendMessage(chatId, { text: `âŒ Speed test failed: ${e.message}`, contextInfo }, { quoted: message });
        }
    }
};
