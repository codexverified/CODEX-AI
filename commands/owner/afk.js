...
module.exports = {
    commands:    ['afk', 'back'],
    category: 'owner',
    description: 'Owner AFK mode â€” bot auto-replies to everyone while you are away',
    permission:  'owner',
    group:       true,
    private:     true,
    ...
    run: async (sock, message, args, ctx) => {
        ...
        if (cmdText === 'back') {
            if (!afkActive) {
                await sock.sendMessage(message.key.remoteJid, { text: 'âœ… AFK mode is not currently active.', contextInfo }, { quoted: message });
                return;
            }
            const duration = formatDuration(Date.now() - afkSince);
            afkActive = false;
            await safeSend({
                text: `ðŸŒ¸ *Welcome back!*\n\nâ± You were away for *${duration}*.`,
                contextInfo
            }, { quoted: message });
        }
    }
