...
// â”€â”€â”€ Strategy 1: cobalt.tools (primary â€” actively maintained public API) â”€â”€â”€â”€â”€â”€â”€
async function tryCobalt(url) {
    ...
    const d = res.data;
    // status: 'redirect' | 'stream' â†’ single video URL
    // status: 'picker' â†’ array of items
...
// â”€â”€â”€ Strategy 2: snapsave.app (supports Facebook + Instagram) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€â”€ Strategy 3: fdown.net â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€â”€ Strategy 4: getvideourl.com â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€â”€ Strategy 5: fdownloader.net â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    run: async (sock, message, args, { sender, prefix, contextInfo }) => {
        ...
        if (!url) {
            return sock.sendMessage(sender, {
                text: `âœ³ï¸ Please send a Facebook video link.\n\nExample: ${prefix}fb https://www.facebook.com/...`,
                contextInfo,
            }, { quoted: message });
        }
        ...
        if (!urlRegex.test(url)) {
            return sock.sendMessage(sender, {
                text: 'âš ï¸ Please provide a valid Facebook URL.',
                contextInfo,
            }, { quoted: message });
        }
 
        await sock.sendMessage(sender, { text: 'ðŸ“¥ Fetching Facebook video...', contextInfo }, { quoted: message });
        ...
        for (const { name, fn } of STRATEGIES) {
            try {
                ...
                if (r?.videoUrl) {
                    videoUrl = r.videoUrl;
                    title    = r.title || title;
                    console.log(`[FB] âœ“ ${name}`);
                    break;
                }
            } catch (e) {
                const msg = String(e?.message || e).slice(0, 80);
                errors.push(`${name}: ${msg}`);
                console.warn(`[FB] âœ— ${name}: ${msg}`);
            }
        }
        ...
        if (!videoUrl) {
            return sock.sendMessage(sender, {
                text:
                    `âŒ *Facebook Download Failed*\n\n` +
                    `All sources failed â€” this happens with private or restricted videos.\n\n` +
        ...
        try {
            await sock.sendMessage(sender, {
                video:   { url: videoUrl },
                caption: `ðŸ¦‹ *Facebook*  â€¢  ðŸ“Œ ${title}`,
                contextInfo: {
                    ...contextInfo,
                    externalAdReply: {
