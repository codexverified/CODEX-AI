    ...
    run: async (sock, message, args, ctx) => {
        ...
        if (!query) {
            return sock.sendMessage(jid, {
                text: `âŒ Provide a title to search.\n\nExamples:\nâ€¢ \`.anime Naruto\`\nâ€¢ \`.manga One Piece\``,
                contextInfo
            }, { quoted: message });
        }
        ...
        try {
            ...
            if (!item) {
                return sock.sendMessage(jid, { text: `âŒ No results found for "*${query}*".`, contextInfo }, { quoted: message });
            }
 
            const title   = item.title_english || item.title;
            const japanese = item.title_japanese || '';
            const score   = item.score ? `â­ ${item.score}/10` : 'N/A';
            ...
            const truncSynopsis = synopsis.length > 400 ? synopsis.slice(0, 400) + 'â€¦' : synopsis;
            const url     = item.url || '';
            ...
            await sock.sendMessage(jid, {
                text: `âŒ Search failed: ${e.message}`,
                contextInfo
            }, { quoted: message });
        }
    }
