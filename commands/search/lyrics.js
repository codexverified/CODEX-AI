...
// lyrics.ovh is dead (ECONNREFUSED / no response) â€” removed.
...
module.exports = {
    ...
    run: async (sock, message, args, ctx) => {
        ...
        try {
            ...
            return sock.sendMessage(jid, {
                text: `ðŸŽµ *${trackTitle}* â€” ${trackArtist}\n\n${lyrics}${trunc}`,
                contextInfo
            }, { quoted: message });
        } catch {}
            ...
            if (hit?.url) {
                ...
                if (match?.length) {
                    ...
                    if (raw.length > 20) {
                        ...
                        const trunc  = raw.length > 3500 ? '\n_[â€¦truncated]_' : '';
                        return sock.sendMessage(jid, {
                            text: `ðŸŽµ *${hit.title || title}* â€” ${hit.primary_artist?.name || artist || 'Unknown'}\n\n${lyrics}${trunc}`,
                            contextInfo
                        }, { quoted: message });
                    }
                }
        ...
        await sock.sendMessage(jid, {
            text:
                ...
                `ðŸ”— Genius: https://genius.com/search?q=${geniusQ}\n` +
                `ðŸ”— AZLyrics: https://www.azlyrics.com/lyrics.html?q=${azQ}\n` +
                `ðŸ”— Musixmatch: https://www.musixmatch.com/search/${geniusQ}`,
            contextInfo
        }, { quoted: message });
    }
};
