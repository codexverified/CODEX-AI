    ...
    run: async (sock, message, args, { sender, groupId, contextInfo }) => {
        ...
        if (!query) {
            return sock.sendMessage(chatId, {
                text: 'ðŸ” Usage: .stickersearch <query>\nExample: .stickersearch laughing cat',
                contextInfo
            }, { quoted: message });
        }
        await sock.sendMessage(chatId, { text: `â³ Searching GIFs for "${query}"...`, contextInfo }, { quoted: message });
        try {
            ...
            await sock.sendMessage(chatId, {
                ...
                gifPlayback: true,
                mimetype:  'video/mp4',
                caption:   `ðŸŽ­ *${query}* â€” CODEX AI`,
                contextInfo
            }, { quoted: message });
        } catch (e) {
            await sock.sendMessage(chatId, { text: `âŒ Sticker search failed: ${e.message}`, contextInfo }, { quoted: message });
        }
    }
};
