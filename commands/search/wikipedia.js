...
module.exports = {
    ...
    run: async (sock, message, args, ctx) => {
        ...
        try {
            ...
            const trunc   = (extract?.length || 0) > 1200 ? '\n_[â€¦read more on Wikipedia]_' : '';
            if (thumbnail?.source) {
                await sock.sendMessage(jid, {
                    image:   { url: thumbnail.source },
                    caption: `ðŸ“– *${displaytitle}*\n\n${summary}${trunc}\n\nðŸ”— ${pageUrl}`,
                    contextInfo
                }, { quoted: message });
            } else {
                await sock.sendMessage(jid, {
                    text: `ðŸ“– *${displaytitle}*\n\n${summary}${trunc}\n\nðŸ”— ${pageUrl}`,
                    contextInfo
                }, { quoted: message });
            }
        } catch {
            await sock.sendMessage(jid, { text: `âŒ Nothing found for *"${query}"*.`, contextInfo }, { quoted: message });
        }
    }
};
