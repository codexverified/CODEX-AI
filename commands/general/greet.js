...
// â”€â”€ Once-per-day tracker (in-memory, resets on bot restart / midnight) â”€â”€â”€â”€â”€â”€â”€â”€
// Key: senderJid  â†’  Value: 'YYYY-MM-DD' string of the last date greeted.
...
function isEnabled() {
    // Default ON â€” becomes OFF only if explicitly set false
...
// â”€â”€ Bootstrap from GREETING env var â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
module.exports = {
    ...
    async run(sock, message, args, ctx) {
        ...
        // â”€â”€ Toggle ON â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        if (cmd === 'greeton') {
            ...
            return reply(fmt(
                ...
                (txt
                    ? `ðŸ“ Current message:\n_"${txt}"_`
                    : `âš ï¸ No greeting text set yet.\nUse \`.setgreet <message>\` to set one.`)
            ));
        }
 
        // â”€â”€ Toggle OFF â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        if (cmd === 'greetoff') {
            greetData['__enabled__'] = false;
            saveData(greetData);
            return reply(fmt('âŒ *Greeting disabled.*\n\nPeople who message the bot will not receive an auto-reply.'));
        }
 
        // â”€â”€ Set greeting text â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        if (cmd === 'setgreet') {
            const text = args.join(' ').trim();
            if (!text) {
                return reply(fmt(
                    `ðŸ“ *Usage:* \`.setgreet <message>\`\n\n` +
                    `_Example:_\n\`.setgreet Hey! I'm busy right now, I'll reply soon ðŸ˜Š\`\n\n` +
        ...
        // â”€â”€ View current greeting â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        if (cmd === 'getgreet') {
            const txt = getGreetText();
            const on  = isEnabled();
            if (!txt) {
                return reply(fmt('âŒ No greeting set.\n\nUse `.setgreet <message>` to set one.'));
            }
            return reply(fmt(
                `ðŸ“ *Current Greeting*\n\n` +
                `_"${txt}"_\n\n` +
                `Status: ${on ? 'âœ… *ON* â€” sent once per day' : 'âŒ *OFF* â€” paused'}\n\n` +
                `â€¢ \`.greeton\` / \`.greetoff\` â€” toggle\n` +
                `â€¢ \`.setgreet <msg>\` â€” change message\n` +
                `â€¢ \`.delgreet\` â€” delete`
            ));
        }
 
        // â”€â”€ Delete greeting â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        if (cmd === 'delgreet') {
            const txt = getGreetText();
            if (!txt) return reply(fmt('âŒ No greeting to delete.'));
            ...
            saveData(greetData);
            greetedToday.clear();
            return reply(fmt('âœ… Greeting *deleted*.'));
        }
    },
 
    // â”€â”€ onMessage hook â€” fires for every incoming private message â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
