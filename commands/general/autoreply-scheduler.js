    ...
    run: async (sock, message, args, ctx) => {
        ...
        if (!args.length || args.length < 2) {
            ...
            if (current && current.enabled) {
                status = `âœ… Active: ${current.start} â€” ${current.end}\nðŸ“ "${current.message}"`;
            }
            return sock.sendMessage(sender, {
                text: `â° *Auto-Reply Scheduler*\n\n*Status:* ${status}\n\n*Usage:*\nâ€¢ \`.autoreply 22:00 08:00 I'm sleeping!\`\nâ€¢ \`.awayoff\` â€” disable\n\n*Time format:* 24-hour (HH:MM)\nOvernight schedules work too (22:00 to 08:00).`,
                contextInfo
            }, { quoted: message });
        }
        ...
        if (!timeRegex.test(startTime) || !timeRegex.test(endTime)) {
            return sock.sendMessage(sender, {
                text: 'âŒ Invalid time format. Use HH:MM (24-hour).\n\nExample: `.autoreply 22:00 08:00 Sleeping!`',
                contextInfo
            }, { quoted: message });
        }
