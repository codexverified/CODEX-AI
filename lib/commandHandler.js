...
// Inlined from the now-removed lib/theme.js — this is only ever read by
// Silva-style plugins via ctx.theme (see the ctx object built in
// loadPluginFile() below); it never depended on anything else in
// lib/theme.js, so it's copied here verbatim rather than kept as a
// separate module.
const theme = {
    botName: process.env.BOT_NAME || 'CODEX AI',
    admin: 'Admin only commands!.',
    botAdmin: 'Admin privilege required !.',
...
class CommandHandler {
    constructor(bot) {
        this.bot         = bot;
        // Anchored to the project root, matching lib/pluginManager.js's
        // PROJECT_ROOT convention exactly (CODEX_PROJECT_ROOT overrides for
        // tests only; production never sets it, so this resolves to the
        // same real install directory as before). Previously this used
        // __dirname directly and ignored the env override, which silently
        // diverged from pluginManager.js's PLUGINS_DIR in any test/tooling
        // context that sets CODEX_PROJECT_ROOT — commands installed via
        // pluginManager landed in one directory while this class scanned a
        // different one. Harmless in production (both resolved to the same
        // path anyway) but meant plugin-persistence behavior couldn't be
        // verified in isolation without touching the real repo's plugins/.
        const PROJECT_ROOT = process.env.CODEX_PROJECT_ROOT || path.join(__dirname, '..');
        this.commandsDir = path.join(PROJECT_ROOT, 'commands');
        this.pluginsDir  = path.join(PROJECT_ROOT, 'plugins');
    }
        ...
        for (const category of categories) {
            ...
            for (const file of files) {
                try {
                    ...
                    // BUG (was): error swallowed entirely — "failed to load"
                    // gave no way to find out WHY, for anyone, ever. Logging
                    // the file + real error here is the whole difference
                    // between guessing and actually fixing a load failure.
                    failed++;
                    console.log(chalk.red(`[commandHandler] failed to load ${category}/${file}: ${err.message}`));
                }
            }
        }
    ...
    loadPlugins() {
        ...
        for (const filePath of files) {
            try {
                ...
                failed++;
                console.error(`[plugin-load] ${path.relative(this.projectRoot || path.join(__dirname, '..'), filePath)}: ${err?.stack || err}`);
            }
        }
            ...
            const command = {
                ...
                execute: async (sock, message, args, cmdName = '') => {
                    ...
                    const ctx = {
                        ...
                        contextInfo: message.msg?.contextInfo || message.message?.extendedTextMessage?.contextInfo || {},
                        groupMetadata,
