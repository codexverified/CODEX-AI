...
// ── Rate limiting ────────────────────────────────────────────────────────
// Previously ask() would hit the external AI API on every single triggered
// chatbot message with no throttling — a group chat that pings the bot
// rapidly (or a misbehaving trigger loop) could burn through API quota in
// seconds and hit provider-side rate limits with no local fallback or
// warning. This caps each conversation key to a modest number of calls per
// minute; requests beyond that return null immediately (callers already
// treat a null reply as "couldn't respond") instead of hitting the API.
const RATE_LIMIT_MAX = 8;
const RATE_LIMIT_WINDOW_MS = 60 * 1000;
const _rateLimitHits = new Map(); // key -> array of call timestamps

function _isRateLimited(key) {
    if (!key) return false;
    const now = Date.now();
    const hits = (_rateLimitHits.get(key) || []).filter((t) => now - t < RATE_LIMIT_WINDOW_MS);
...
async function ask({ bot, key, system = '', user, remember = true }) {
    ...
    if (_isRateLimited(key || 'global')) {
        console.error('[smartAI] rate limit hit for', key || 'global', '— skipping API call');
        return null;
    }
