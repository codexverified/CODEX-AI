...
const DIR = path.join(PROJECT_ROOT, 'database', 'media-store');
const INDEX_FILE = path.join(PROJECT_ROOT, 'database', 'media-store-index.json');

// Cap how many files we keep on disk so this can't grow unbounded.
// Oldest entries are pruned first. 300 media files (images/videos/voice
// notes) can easily add up to hundreds of MB — 50 is enough to cover the
// realistic anti-delete recovery window (someone deleting media within
// minutes/hours of sending it) without risking disk fill on busy hosts.
const MAX_FILES = 50;
// Age-based pruning: also delete anything older than this regardless of
// count, so a quiet period doesn't leave stale files sitting around for
// weeks just because MAX_FILES was never hit.
...
function prune(idx) {
  ...
  // Age-based pruning first — independent of count.
  const stale = keys.filter((k) => now - (idx[k].ts || 0) > MAX_AGE_MS);
  for (const k of stale) {
    const file = idx[k].file;
    delete idx[k];
    if (file) fs.unlink(path.join(DIR, file), () => {});
  }

  // Then count-based pruning for whatever's left.
  const remaining = Object.keys(idx);
  if (remaining.length <= MAX_FILES) return;
  const sorted = remaining.sort((a, b) => (idx[a].ts || 0) - (idx[b].ts || 0));
  const toRemove = sorted.slice(0, remaining.length - MAX_FILES);
