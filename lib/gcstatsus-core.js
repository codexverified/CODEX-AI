 ...
 *  This Baileys fork (@crysnovax/baileys) reads `groupStatus`, `backgroundColor`
 *  and `font` all off the message CONTENT (2nd arg) — none of them belong in
 *  the send-options object, unlike upstream/mainline Baileys where
 *  backgroundColor/font are options. Putting them in options here means they
 *  get silently dropped: the post still goes through, just without the color.
...
async function postGroupStatus(sock, groupJid, content) {
    ...
    // The manual relay builds the proto directly, so set these fields on
    // the generated extended-text message ourselves.
...
function parsePipeArgs(raw) {
    ...
    for (const seg of segments.slice(1)) {
        ...
        const namedColor = COLORS[seg.toLowerCase()];
        if (namedColor)                   { color = namedColor; continue; }
