...
const ID_DB   = path.join(PROJECT_ROOT, 'database/gstatus-ids.json');
 ...
 *  `backgroundColor` must be passed through the send options so
...
async function postGroupStatus(sock, groupJid, content) {
    const { backgroundColor, ...messageContent } = content;
...
/** Splits text plus an optional background color and target into its parts. */
function parsePipeArgs(raw) {
    ...
    for (const seg of segments.slice(1)) {
        ...
        const match = seg.match(/^(bg|background):(.+)$/i);
        ...
        if (!color) continue;
        if (kind === 'bg' || kind === 'background') backgroundColor = color;
        else if (!kind && !backgroundColor) backgroundColor = color;
        // unrecognized segment — ignored rather than treated as an error
    }
    return { text, backgroundColor, target };
}
