...
async function postGroupStatus(sock, groupJid, content) {
    ...
    const hasColoredText = typeof messageContent.text === 'string' &&
        (backgroundColor !== undefined || normalizedTextColor !== undefined);

    if (!hasColoredText) {
        try {
            return await sock.sendMessage(
                groupJid,
                { ...messageContent, backgroundColor, font, groupStatus: true },
    ...
    // The high-level `groupStatus:true` shortcut stamps contextInfo.isGroupStatus
    // on the extendedTextMessage itself (not just the outer envelope) — without
    // it WhatsApp still posts the status but renders it with a default black
    // background instead of the requested color. This manual relay path is the
    // fallback used whenever the shortcut throws, so it needs to do the same.
    if (inner?.extendedTextMessage) {
...
/** Splits text plus optional bg:/fg: colors and a target into its parts. */
function parsePipeArgs(raw) {
    ...
    return { text, backgroundColor, textColor, target };
}
