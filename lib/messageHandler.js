...
class MessageHandler {
  ...
  async handle(msg) {
    try {
      ...
      // Passively builds up presence data for .listonline/.listoffline —
      // fire-and-forget, throttled internally (see ensureSubscribed), so
      // this never blocks or slows down normal message handling.
      if (m.isGroup) {
        try { require('./presenceStore').ensureSubscribed(this.bot.sock, jid).catch(() => {}); } catch {}
      }
