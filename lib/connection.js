  ...
  if (!hasSession && phoneNumber) {
    ...
    const requestCodeWithRetry = async (attempt = 1) => {
      ...
      try {
        ...
        console.log(
          chalk.yellow(
            "Enter it on your phone within ~60s. If it says \"couldn't link device\", the code expired — a fresh one will be generated automatically; wait for it instead of reusing the old one.",
          ),
        );
      } catch (err) {
