  ...
  bot.sock.ev.on("connection.update", async (update) => {
    ...
    if (connection === "close") {
      ...
      if (isRestartRequired) {
        ...
        _scheduleReconnect(bot, 0, "restart-required (pairing handshake)");
        return;
      }
      ...
      bot._connectionHeartbeat.unref?.();
      ...
      async function _probeSocketHealth() {
        if (typeof liveSocket.query === "function") {
          // A minimal, cheap IQ round-trip against WhatsApp's own servers —
          // any valid reply (including an error stanza) proves the socket
          // is genuinely carrying traffic, not just locally "open".
        ...
        // Fallback for socket shapes without .query — still better than
        // nothing, but kept as a fallback because (as above) it cannot by
        // itself prove the round trip completed.
