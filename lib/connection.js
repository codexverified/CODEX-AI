...
const CONNECTION_WATCHDOG_MS = 60000;
...
async function _startConnection(bot) {
  ...
  bot.sock.ev.on("connection.update", async (update) => {
    ...
    if (connection === "close") {
      ...
      // A socket can remain "open" in WhatsApp's linked-devices list while
      // its WebSocket has silently stopped carrying traffic. The log heartbeat
      // cannot detect that state, so probe the live socket and force the normal
      // reconnect path when WhatsApp rejects the probe.
      if (bot._connectionWatchdog) clearInterval(bot._connectionWatchdog);
      const liveSocket = bot.sock;
      bot._connectionWatchdog = setInterval(async () => {
        if (bot.sock !== liveSocket || !liveSocket?.user) return;
        try {
          if (typeof liveSocket.sendPresenceUpdate !== "function") return;
          await liveSocket.sendPresenceUpdate("available", liveSocket.user.id);
        } catch (error) {
          console.error(`[connection watchdog] probe failed: ${error?.message || error}`);
          clearInterval(bot._connectionWatchdog);
          bot._connectionWatchdog = null;
          try { liveSocket.end(error); } catch {}
          startConnection(bot).catch((err) =>
            console.error("[connection watchdog] reconnect failed:", err?.message || err),
          );
        }
