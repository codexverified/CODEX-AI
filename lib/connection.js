...
const SESSION_META_PATH = path.resolve(process.cwd(), "session", ".sessionId");
  ...
  bot.sock.ev.on("connection.update", async (update) => {
    ...
    if (connection === "close") {
      ...
      async function _probeSocketHealth() {
        // sendPresenceUpdate() is used as the primary probe. sock.query()
        // registers an internal listener for a matching IQ response and, on
        // a half-open socket, that promise can simply never resolve —
        // _withTimeout() below bounds it, but a hung query still leaves a
        // dangling listener/promise on the old socket. sendPresenceUpdate()
        // is a normal stanza write through the same websocket, so a socket
        // that's actually dead will throw (or the surrounding timeout will
        // catch it) without leaving that residue behind.
      ...
      async function watchdogCycle() {
        if (bot.sock !== liveSocket || !liveSocket?.user) return;
        ...
        try {
          ...
          bot._watchdogMisses = 0;
          _logConn("watchdog probe ok", { lastEventAgoMs });
        } catch (error) {
