...
const ROOT = path.join(PROJECT_ROOT, 'database');
const MESSAGE_DB = path.join(ROOT, 'message-store.json');
const MEDIA_DB = path.join(ROOT, 'media-store.json');
// Was 3000 — that many full message objects (needed for anti-delete/
// anti-edit to show "what was the original message") is a meaningful chunk
// of memory and disk on a busy multi-group bot. 200 still covers the
// realistic anti-delete/anti-edit window (a message getting edited/deleted
// minutes after being sent) without the unbounded growth.
