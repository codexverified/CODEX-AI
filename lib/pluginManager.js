...
function listPlugins(bot) {
    // Anything loaded from lib/commandHandler.js's loadPlugins() (i.e. from
    // the plugins/ directory) gets __plugin = true — that alone is already
    // the correct, exhaustive signal that a command is an installed plugin
    // rather than one of the bot's own built-in commands (built-ins live
    // under commands/ and are never given __plugin).
    //
    // This used to ALSO require command.source (the "// @source: <url>"
    // marker embedded by saveAndLoad()) before a plugin would show up here.
    // That's wrong: command.source is only set when the file happens to
    // carry that comment. A plugin dropped into plugins/ any other way —
    // installed by an older build of .install before the marker existed,
    // uploaded manually over (S)FTP/the panel's file manager, restored from
    // a backup, or written by hand — has no source comment and was being
    // silently excluded from .listplugins/.install list/.uninstall list
    // even though bot.commands and the startup "N plugin commands loaded"
    // count (see app.js) both show it as present and working. That's
    // exactly the "plugin works but .listplugins says none installed" bug:
    // the command ran fine (it's genuinely registered), the LIST just
    // filtered it out. Showing the source link is still nice-to-have where
    // we have it, so callers should render `cmd.source` only when present,
    // not use its absence to hide the plugin entirely.
    const seen = new Set();
    const plugins = [];
