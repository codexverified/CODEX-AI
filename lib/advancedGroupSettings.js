// Advanced Group Settings — .low / .smart / .hyper
//
// Mass-toggles every per-group anti-system that goes through
// AntiSystems#_doAction() (lib/antiSystems.js) — the same enabled/action/
// maxWarns shape used by .antilink, .antispam, .antitag, etc. — so a
// single command sets them all at once, and each system's own `.<name>
// status` command reads the exact same file/key and immediately reflects
// the change.
//
// Anti-GC-Status is pinned to 'kick' in every mode (forceAction below) —
// per spec, it never follows low/smart.
// Anti-Fake has no 'delete' action (a join event has no message to
// delete), so LOW leaves its action untouched and just reports that.
const fs = require('fs-extra');
const path = require('path');

// Anchored to the project root (not process.cwd()) so this lands in the
// same place regardless of the directory the process was launched from.
// CODEX_PROJECT_ROOT is a test-only override; production never sets it.
const PROJECT_ROOT = process.env.CODEX_PROJECT_ROOT || path.join(__dirname, '..');

const SYSTEMS = [
  { key: 'dnd',              label: 'DND',                file: 'dnd.json',              default: { enabled: false, action: 'delete', maxWarns: 3, customMsg: null },        supportsDelete: true },
...
function applyMode(groupId, mode, maxWarns = 3) {
  ...
  for (const sys of SYSTEMS) {
    ...
    // Preserve any extra fields already on this group's entry (antiword's
    // banned-words list, antispam's limit/cooldown, dnd's customMsg, ...) —
    // only enabled/action/maxWarns are touched here.
...
module.exports = { SYSTEMS, applyMode };
