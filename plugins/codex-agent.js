...
const intentMap = [
    ...
    // ── GitHub CodexAI ────────────────────────────────────────────────────
    { pattern: /\bcodexai\b.*\b(repos?|github|projects?)\b|\b(codexai|codexairepos)\b/i, cmd: 'codexai', label: '🐙 Fetching CodexAI repos',  strip: /\bcodexai\b|\bcodexairepos?\b/gi },
...
const smartResponses = [
    { p: /^(hi+|hello+|hey+|howdy|sup|yo+|hii+|ello)\b/i,
      r: [`Hey! 👋 What can I do for you today?`, `Hello! 😊 How can I help you?`, `Hey there! I'm Codex, your WhatsApp assistant. What do you need? 🤖`] },
    ...
    { p: /what('?s| is) your name|your name\b|who are you\b/i,
      r: [`I'm *Codex* 🤖 — your intelligent WhatsApp assistant! Built on ${BOT_IDENTITY.name} v${BOT_IDENTITY.version}.`] },
    { p: /what can you do|your (capabilities|powers|features|abilities)\b/i,
      r: [`I can: 🎵 play music, 📸 make stickers, ⬇️ download from TikTok/Instagram/YouTube, 🌤️ check weather, 📚 search Wikipedia, 🌐 translate text, 👥 manage groups, and 1400+ commands! Type \`codex help\` to see everything.`] },
...
function offlineSmartReply(query, pluginKeys) {
    ...
    const catchAll = [
        ...
        `Codex here! 💬 My response engine is warming up. One more try should do it — or use \`.menu\` to browse all commands!`,
    ];
...
module.exports = {
    ...
    run: async (sock, message, args, ctx) => {
        ...
        if (!query) return reply(
            ...
            `🎵 *Music & Media*\n` +
            `• "codex play <song>" • "codex lyrics <song>"\n` +
            `• "codex tiktok <url>" • "codex youtube video <name>"\n` +
            `• "codex speak Hello world" _(text-to-speech)_\n\n` +
            `🎨 *AI & Images*\n` +
            `• "codex generate image of a lion in space"\n` +
            `• "codex describe" _(reply to any photo)_\n` +
            `• "codex summarize" _(reply to a long message)_\n` +
            `• "codex quotly" _(reply to a message → quote sticker)_\n\n` +
            `🧩 *Fun & Knowledge*\n` +
            `• "codex riddle" — get a brain teaser\n` +
            `• "codex proverb" / "codex wisdom" — wise saying\n` +
            `• "codex rhyme moon" — words that rhyme\n\n` +
            `🐙 *GitHub*\n` +
            `• "codex codexai repos" — list CodexAI repos\n` +
            `• ".codexai <repo>" — repo stats & files\n` +
            `• ".codexai zip <repo>" — download link\n\n` +
            `📲 *Downloads*\n` +
            `• "codex instagram <url>" • "codex facebook <url>"\n` +
            `• "codex spotify <name>" • "codex pinterest <query>"\n\n` +
            `⏰ *Productivity*\n` +
            `• "codex remind me in 30m to call mom"\n` +
            `• "codex save note shopping: milk, eggs"\n` +
            `• "codex get my notes"\n` +
            `• "codex create a poll: Question | A | B | C"\n` +
            `• "codex schedule message at 9pm: team meeting"\n\n` +
            `🛠️ *Tools*\n` +
            `• "codex sticker" _(reply to photo)_\n` +
            `• "codex translate hello to french"\n` +
            `• "codex wiki artificial intelligence"\n` +
            `• "codex weather Nairobi"\n` +
            `• "codex qr code https://example.com"\n\n` +
            `👥 *Group Management*\n` +
            `• "codex change group name to X"\n` +
            `• "codex mute/unmute group"\n` +
            `• "codex tag all members"\n` +
            `• "codex kick @user" • "codex promote @user"\n` +
            `• "codex create a poll: Vote | Yes | No"\n\n` +
            `✍️ *Content Creation*\n` +
            `• "codex create a bio / welcome / rules / poem"\n` +
            `• "codex write an email about X"\n\n` +
            `🌐 *Info & Web*\n` +
            `• "codex weather / news / search / ip info"\n` +
            `• "codex about bot / platform / owner / settings"\n\n` +
            `🧠 *AI Chat* — Ask me anything! I remember our conversation.\n` +
            `• "codex forget" — clears chat memory\n\n` +
            `📋 *Run Any Command:* "codex run <command>"\n\n` +
        ...
        if (agentActions.about_bot.test(query)) {
            ...
            response =
                `👑 *Bot Owner*\n\n` +
                `• Name: *${config.OWNER_NAME || 'CODEX AI'}*\n` +
                `• Number: *+${(config.OWNER_NUMBER || '').replace(/\D/g, '')}*\n` +
                `• Bot: *${config.BOT_NAME || 'CODEX AI'}*\n` +
            ...
            response =
                ...
                `👥 *Group Management*\n` +
                `• "codex change group name to X"\n` +
                `• "codex set group description to X"\n` +
                `• "codex mute/unmute group"\n` +
                `• "codex lock/unlock group"\n` +
                `• "codex get group link"\n` +
                `• "codex tag all members"\n` +
                `• "codex list admins"\n` +
                `• "codex kick/add/promote/demote"\n\n` +
                `🌐 *Web Access*\n` +
        ...
        // of how the message was triggered (no rawCmd detection issues).
            ...
            if (!mentioned) return reply(`💡 Reply to a member's message and say: _agent kick_\n\nOr tag them: _agent kick @member_`);
            try {
            ...
            const mentioned = ctx.mentionedJid?.[0] || message.message?.extendedTextMessage?.contextInfo?.participant;
            if (!mentioned) return reply(`💡 Reply to a member's message and say: _agent promote_\n\nOr tag them: _agent promote @member_`);
            try {
            ...
            const mentioned = ctx.mentionedJid?.[0] || message.message?.extendedTextMessage?.contextInfo?.participant;
            if (!mentioned) return reply(`💡 Reply to a member's message and say: _agent demote_\n\nOr tag them: _agent demote @member_`);
            try {
        ...
        // ── GitHub repo ZIP download ("send me a clip of owner/repo") ──
        ...
        // ── GitHub repo file fetch ────────────────────────────────────────────
            ...
            if (!repoMatch) {
                return reply(`📄 Specify the repo and file.\n\nExamples:\n• _agent get README from owner/repo_\n• _.ghrepo owner/repo README.md_`);
            }
        ...
        // ── Rhyme dispatch ────────────────────────────────────────────────────
            ...
            if (smart) {
                    ...
                    try {
                        ...
                        if (apiKey) {
                            ...
                            const geminiHistory = [
                                ...
                                { role: 'model', parts: [{ text: `Got it! I'm Codex, ready to help. 🤖` }] },
                                ...mem.slice(0, -1).map(m => ({
                ...
                response = aiReply.startsWith('🤖 *Codex*') ? aiReply : `🤖 *Codex*\n\n${aiReply}`;
