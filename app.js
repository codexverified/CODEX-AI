...
class CODEXAI {
  ...
  // Retries acquiring the session lock for up to ~15s (1s apart) before
  ...
  async _acquireProcessLockWithRetry(sessionDir, { retries = 25, intervalMs = 1200 } = {}) {
    for (let attempt = 0; ; attempt++) {
      ...
      console.log(chalk.yellow(
        `[lock] session still held by pid ${result.holderPid} — probably the previous process still shutting down. ` +
