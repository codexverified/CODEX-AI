...
/**
 * Build the <call><offer type="group" group-jid=..> stanza used to start a group call.
 * Pure: no socket state, unit-testable.
 */
export const buildGroupCallStanza = ({ id, to, callId, callCreator, offerContent }) => ({
    tag: 'call',
    attrs: {
        id,
        to
...
/**
 * Build the <call><terminate> stanza used to cancel / hang up a group call.
 * Pure: no socket state, unit-testable.
 */
export const buildGroupCallCancelStanza = ({ to, callId, callCreator }) => ({
    tag: 'call',
    attrs: {
        from: callCreator,
        to
...
export const makeMessagesRecvSocket = (config) => {
    ...
    /**
     * Start a group call. Mirrors the proven 1:1 `offerCall` stanza (encrypted
     * callKey envelope per destination) but addressed to the group with the
     * `type="group"` + `group-jid` offer attributes that the receive pipeline
     * recognizes for group calls.
     * @param jid the group JID (e.g. 120363012345678901@g.us)
     * @param participants participant JIDs to ring
     * @param isVideo whether to offer a video call
     */
    const groupCall = async (jid, participants = [], isVideo = false) => {
        const callId = randomBytes(16).toString('hex').toUpperCase().substring(0, 64);
        const offerContent = [];
        offerContent.push({
            tag: 'audio',
        ...
        const encKey = randomBytes(32);
        const devices = (await getUSyncDevices(participants, true, false)).map(({ user, device }) => jidEncode(user, 's.whatsapp.net', device));
        await assertSessions(devices, true);
        const { nodes: destinations, shouldIncludeDeviceIdentity } = await createParticipantNodes(devices, {
            call: {
                callKey: new Uint8Array(encKey)
        ...
        const stanza = buildGroupCallStanza({
            id: generateMessageID(),
            to: jid,
            callId,
            callCreator: authState.creds.me.id,
            offerContent
        });
        await query(stanza);
        return {
    ...
    /**
     * Cancel / terminate a group call started with `groupCall`.
     * Mirrors the proven 1:1 `terminateCall` stanza addressed to the group.
     * @param jid the group JID the call was started in
     * @param callId the call id returned by `groupCall`
     */
    const cancelGroupCall = async (jid, callId) => {
        const stanza = buildGroupCallCancelStanza({
            to: jid,
            callId,
            callCreator: authState.creds.me.id
        });
    ...
    // Expose the group-call methods on the socket surface. Attached to the inner
    // `sock` so they ride the `...sock` spread of this layer's return object
    // (the return block lives beyond the edit window of the receive pipeline).
    sock.groupCall = groupCall;
    sock.cancelGroupCall = cancelGroupCall;
    ...
    const handleNewsletterStatus = async (node) => {
        ...
        if (type === 'reaction') {
            const reactionNode = getBinaryNodeChild(node, 'reaction');
