...
import { BinaryInfo, encodeWAM } from '../WAM/index.js';
...
export const makeSocket = (config) => {
    ...
    if (printQRInTerminal) {
        logger.warn({}, ' The printQRInTerminal option has been deprecated. You will no longer receive QR codes in the terminal automatically. Please listen to the connection.update event yourself and handle the QR your way. You can remove this message by removing this opttion. This message will be removed in a future version.');
    }
    ...
    if (syncDisabled) {
        logger.warn(' DANGER: DISABLING ALL SYNC BY shouldSyncHistoryMsg PREVENTS BAILEYS FROM ACCESSING INITIAL LID MAPPINGS, LEADING TO INSTABILIY AND SESSION ERRORS');
    }
    ...
    const flushWAMEvents = async () => {
        if (!publicWAMBuffer.events.length)
            return;
        try {
            const encoded = encodeWAM(publicWAMBuffer);
