...
import { DEFAULT_CACHE_TTLS, HISTORY_SYNC_PAUSED_TIMEOUT_MS, PROCESSABLE_HISTORY_TYPES, WA_ADV_HOSTED_KEY_INDEX_LIST_SIG_PREFIX, WA_ADV_KEY_INDEX_LIST_SIG_PREFIX } from '../Defaults/index.js';
import { ALL_WA_PATCH_NAMES } from '../Types/index.js';
import { SyncState } from '../Types/State.js';
import { chatModificationToAppPatch, Curve, decodePatches, decodeSyncdSnapshot, encodeSyncdPatch, ensureLTHashStateVersion, extractSyncdPatches, generateProfilePicture, getHistoryMsg, isAppStateSyncIrrecoverable, isMissingKeyError, MAX_SYNC_ATTEMPTS, newLTHashState, processSyncAction } from '../Utils/index.js';
import { generateProfilePictureHD } from '../Utils/messages-media.js';
import { makeMutex } from '../Utils/make-mutex.js';
import processMessage from '../Utils/process-message.js';
import { buildTcTokenFromJid } from '../Utils/tc-token-utils.js';
import { getBinaryNodeChild, getBinaryNodeChildren, isHostedLidUser, isHostedPnUser, isJidNewsletter, isLidUser, isPnUser, jidDecode, jidNormalizedUser, reduceBinaryNodeToDictionary, S_WHATSAPP_NET } from '../WABinary/index.js';
...
export const makeChatsSocket = (config) => {
    ...
    // Cache newsletter messageId  server_id so quoted posts can be enriched later
    ...
    /**
     * Fetch broadcast list quota from the server (xmlns="w:biz").
     *
    ...
    const fetchBroadcastListQuota = async () => {
        ...
        if (!limitsNode)
            return null;
        return {
            messagesLeft: parseInt(limitsNode.attrs?.messages_left ?? getBinaryNodeChild(limitsNode, 'messages_left')?.content ?? '0', 10),
            totalLimit: parseInt(limitsNode.attrs?.total_limit ?? getBinaryNodeChild(limitsNode, 'total_limit')?.content ?? '0', 10),
            isHeavySender: (limitsNode.attrs?.is_heavy_sender ?? getBinaryNodeChild(limitsNode, 'is_heavy_sender')?.content) === 'true',
            startTs: parseInt(timeframeNode?.attrs?.start_ts_s ?? getBinaryNodeChild(timeframeNode, 'start_ts_s')?.content ?? '0', 10),
            endTs: parseInt(timeframeNode?.attrs?.end_ts_s ?? getBinaryNodeChild(timeframeNode, 'end_ts_s')?.content ?? '0', 10),
            resetTs: parseInt(timeframeNode?.attrs?.reset_ts_s ?? getBinaryNodeChild(timeframeNode, 'reset_ts_s')?.content ?? '0', 10)
        };
    };
    /**
    ...
    const getChatBlockingStatus = async () => {
        ...
        const blocking = getBinaryNodeChild(result, 'blocking') ||
            getBinaryNodeChild(getBinaryNodeChild(result, 'query'), 'blocking');
    ...
    /**
     * Get the user's pending TOS disclosures / notices (xmlns="tos").
    ...
    const getUserDisclosures = async (t = 0) => {
        const result = await query({
            tag: 'iq',
            attrs: { to: S_WHATSAPP_NET, xmlns: 'tos', type: 'get' },
            content: [{ tag: 'get_user_disclosures', attrs: { t: String(t) } }]
        });
        return getBinaryNodeChildren(result, 'notice').map(n => ({ ...n.attrs }));
    };
    /**
    ...
    const acceptTosNotice = async (noticeId, result = '105') => {
        await query({
            tag: 'iq',
            attrs: { to: S_WHATSAPP_NET, xmlns: 'tos', type: 'set' },
            content: [{ tag: 'trackable', attrs: { id: String(noticeId), result: String(result) } }]
        });
    };
    /**
     * Report spam for a chat/group with sample messages (xmlns="spam").
    ...
    const reportSpam = async (jid, messages, spamFlow = 'contact_info_report', subject) => {
        ...
        await query({
            tag: 'iq',
            attrs: { to: S_WHATSAPP_NET, xmlns: 'spam', type: 'set' },
            content: [
                {
                    tag: 'spam_list',
                    attrs: {
    ...
    const getOptOutList = async () => {
        return query({
            tag: 'iq',
            attrs: { to: S_WHATSAPP_NET, xmlns: 'optoutlist', type: 'get' }
        });
    };
    /**
     * Sign a blinded credential for privacy-preserving anonymous stats (xmlns `privatestats`).
    ...
    const signPrivateCredential = async (blindedCredential) => {
        const result = await query({
            tag: 'iq',
            attrs: { to: S_WHATSAPP_NET, xmlns: 'privatestats', type: 'get' },
            content: [
                {
                    tag: 'sign_credential',
                    attrs: { version: '1' },
        ...
        const signedNode = getBinaryNodeChild(result, 'sign_credential');
        const signedBytes = getBinaryNodeChild(signedNode, 'signed_credential');
    ...
    const getPushConfig = async () => {
        const result = await query({
            tag: 'iq',
            attrs: { to: S_WHATSAPP_NET, xmlns: 'urn:xmpp:whatsapp:push', type: 'get' },
            content: [{ tag: 'settings', attrs: {} }]
        });
        return getBinaryNodeChild(result, 'settings');
    };
    /**
     ...
     * @param {string} linkToken call-link token
     * @param {boolean} enabled whether the waiting room is on
     * @param {string} [media] "audio" | "video"
     */
    const toggleCallLinkWaitingRoom = async (linkToken, enabled, media = 'audio') => {
        ...
        const child = getBinaryNodeChild(result, 'waiting_room_toggle');
        return child?.attrs;
    };
    /**
     ...
     * @param {import('../WAUSync/USyncQuery.js').USyncQuery} usyncQuery
     */
    const executeUSyncQuery = async (usyncQuery) => {
        const result = await sock.executeUSyncQuery(usyncQuery);
        if (!result)
            return result;
        // auto-persist privacy tokens found in any usync response
        ...
        for (const entry of [...(result.list || []), ...(result.sideList || [])]) {
            ...
            // emit contacts.update for contacts that blocked us
    ...
    /**
     * Fetch AB-test (abt) props from server (xmlns="abt").
     * @param {string} [protocol] e.g. "1" or "2"
     * @param {string} [hash]
     * @param {string|null} [refreshId]
     * @param {string|null} [group] group-scoped query when set
     */
    const fetchABProps = async (protocol = '1', hash = '', refreshId = null, group = null) => {
        const propAttrs = group != null
            ? { group: String(group) }
            : {
                protocol,
                ...(hash ? { hash } : {}),
        ...
        const propsNode = getBinaryNodeChild(result, 'props');
        if (!propsNode)
            return {};
        return reduceBinaryNodeToDictionary(propsNode, 'prop');
    };
    /**
     * Remove a companion (linked) device from the account (xmlns="md").
     * @param {string} jid full device JID of the companion to remove
     * @param {string} [reason] e.g. "user_initiated" | "unknown_companion"
     */
    const removeCompanionDevice = async (jid, reason = 'user_initiated') => {
        await query({
            tag: 'iq',
            attrs: { to: S_WHATSAPP_NET, xmlns: 'md', type: 'set' },
            content: [
                {
    ...
    /**
     * Push an updated key-index-list to the server (xmlns="md", child "key-index-list").
     * @param {number|string} ts unix timestamp seconds
     * @param {Buffer} content serialized proto KeyIndexList
     */
    ...
    /**
     * Build and push the companion's signed key-index-list on (re)connect (xmlns="md").
     * Mirrors CompanionDeviceAdvUtil.A02 from the APK: the details proto is signed with
     * our own device identity key (the same key that produces the ADV deviceSignature).
     */
    const sendKeyIndexList = async () => {
        ...
        const accountType = deviceIdentity.accountType ?? proto.ADVEncryptionType.E2EE;
        ...
        const sigPrefix = isHosted ? WA_ADV_HOSTED_KEY_INDEX_LIST_SIG_PREFIX : WA_ADV_KEY_INDEX_LIST_SIG_PREFIX;
        const accountSignature = Curve.sign(signedIdentityKey.private, Buffer.concat([sigPrefix, details]));
        ...
        const content = proto.ADVSignedKeyIndexList.encode(signedKeyIndexList).finish();
        await updateKeyIndexList(ts, Buffer.from(content));
        logger.info({ ts, keyIndex }, 'sent key-index-list');
    };
    /**
     * Request a media upload connection token from the server (xmlns="w:m").
    ...
    const fetchMediaConn = async (lastId = null) => {
        ...
        const result = await query({
            tag: 'iq',
            attrs: { to: S_WHATSAPP_NET, xmlns: 'w:m', type: 'set' },
            content: [{ tag: 'media_conn', attrs }]
        });
        return getBinaryNodeChild(result, 'media_conn') || null;
    };
    /**
     * Delete a broadcast list by ID (xmlns="w:b").
     */
    const deleteBroadcastList = async (listId) => {
        await query({
            tag: 'iq',
            attrs: { to: S_WHATSAPP_NET, xmlns: 'w:b', type: 'set' },
            content: [
                {
                    tag: 'delete',
                    attrs: {},
    ...
    /**
     * Fetch a QR code from the server (e.g. for linked-device linking) (xmlns="w:qr").
     * @param {string} code
     * @param {'lid'|null} [addressingMode] set to "lid" for LID-addressed QR
     */
    const fetchQRCode = async (code, addressingMode = null) => {
        ...
        const result = await query({
            tag: 'iq',
            attrs: { to: S_WHATSAPP_NET, xmlns: 'w:qr', type: 'get' },
            content: [{ tag: 'qr', attrs }]
        });
        return getBinaryNodeChild(result, 'qr') || null;
    };
    /**
     * Confirm or deny a device-logout request from the server (xmlns="w:account_defence").
     * @param {string|number} id the challenge id
     * @param {boolean} [approve] true to confirm the logout, false to deny
     */
     ...
     *
     * Standard (crop + resize to 720720):
    ...
    const updateProfilePicture = async (jid, content, dimensions) => {
        ...
        // codex@HD-Profile --- Dual mode:
        //   hd:true   generateProfilePictureHD (preserves aspect ratio, caps at 1920, manages file size)
        //   standard  generateProfilePicture (720720 square crop, or custom dimensions)
    ...
    /**
     * codex@Favorites --- Set complete WhatsApp Favorites list.
    ...
    /**
     * codex@Favorites --- Remove all favorites (clear list).
     */
    ...
    const upsertMessage = ev.createBufferedFunction(async (msg, type) => {
        ...
        if (msg.message) {
            ...
            if (ci?.stanzaId && isJidNewsletter(ci.remoteJid)) {
                const sid = _nlServerIdCache.get(ci.stanzaId);
                if (sid != null)
                    msg.quotedNewsletterServerId = sid;
            }
        }
