...
export const generateMessageIDV2 = (userId) => {
   ...
   // Lia@Note 09-04-26 --- Fixed stamp position (not hash-derived) so the
   // marker lands at the same index in every generated ID.
   // CODEX Technology package stamp.
   const pos = 4;
   return baseId.slice(0, pos) + 'CODEX' + baseId.slice(pos);
};
...
export const fetchLatestBaileysVersion = async (options = {}) => {
    const URL = 'https://github.com/CEO-CODEX/baileys-';
    try {
        ...
        if (!response.ok) {
            throw new Boom(`Failed to fetch latest @codexverified/baileys version: ${response.statusText}`, { statusCode: response.status });
        }
