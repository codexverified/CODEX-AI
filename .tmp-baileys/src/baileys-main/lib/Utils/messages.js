...
import { isLidUser, isPnUser, isJidGroup, isJidNewsletter, isJidStatusBroadcast, jidEncode, jidNormalizedUser } from '../WABinary/index.js';
...
const prepareStickerPackMessage = async (message, options) => {
    const { cover, stickers = [], name = ' Sticker Pack', publisher = 'Website: web.codex.link', description = ' @codexverified/baileys' } = message;
...
export const generateWAMessageContent = async (message, options) => {
    ...
    // Lia@Note 03-02-26 --- This message type is shown on WhatsApp Web/Desktop and iOS (I guess ). On Android, it only appears in newsletter (so far )
        ...
        hydratedTemplate.templateId = message.id || 'template-' + Date.now(); // Lia@Note 04-02-26 --- Minimal templateId to satisfy WhatsApp ()
        m = {
            templateMessage: {
    ...
    // codex@VerifiedMe --- Add "verifiedMe" boolean to attach a verified
    ...
    // Lia@Changes 02-02-26 --- Add "interactiveAsTemplate" boolean to wrap interactiveMessage into templateMessage
        ...
        m = {
            templateMessage: {
                ...
                templateId: message.id || 'template-' + Date.now() // Lia@Note 04-02-26 --- Minimal templateId to satisfy WhatsApp ()
            }
        };
        delete message.interactiveAsTemplate;
    }
    // Codex compatibility: expose the public media-spoiler mode without changing
    // the existing explicit viewOnce/viewOnceV2 behavior. The mode is opt-in.
    // Supported values are `viewOnce` and `native` for image, video, document,
    // and sticker messages. Existing `spoiler` and view-once flags take priority.
    if (hasOptionalProperty(message, 'mediaSpoilerMode') &&
        !message.viewOnce &&
        !message.viewOnceV2 &&
        !message.viewOnceV2Extension) {
        const messageType = Object.keys(m)[0];
        const isSpoilerMedia = ['imageMessage', 'videoMessage', 'documentMessage', 'stickerMessage'].includes(messageType);
        const mode = message.mediaSpoilerMode;
        if (isSpoilerMedia && mode === 'viewOnce') {
            m = { viewOnceMessage: { message: m } };
        }
        else if (isSpoilerMedia && mode === 'native') {
            const key = m[messageType];
        ...
        delete message.mediaSpoilerMode;
    }
